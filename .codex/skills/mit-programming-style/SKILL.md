...
description: "コードの実装・変更・リファクタリング、またはソフトウェア設計に使う。一般文章の執筆・校正や資料の構成だけには適用しない。"
---

# MIT-Style Programming Skill

対象のコードと変更に関係する節だけを適用する。命名・責務・テストなどの確認結果は、同一内容・入力・環境ならレビューで再利用し、新しい変更・失敗・未解決事項がある場合のみ再検証する。例示の行数・引数数は目安であり、機械的なリファクタリング条件ではない。
