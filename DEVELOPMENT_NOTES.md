...
## 2026-09-25 分散計算画面の稼働版・修論用本線への反映

- 8868の既存管理画面は、選択シナリオのIDで履歴を絞る／全件へ戻す、監視先ポートを切り替える、8890を閲覧専用で確認する動作を実ブラウザーで確認した。シナリオ切替・再読込時にも監視先を保持し、別ポートのキューを同じ履歴として混ぜない。
- 8890の週次版は `0ef5f301` の静的フロントを、計算用SHA `0964b783` のcontrollerを再起動せず配置した。フロント47テスト・本番ビルド・HTTP配信・実ブラウザーで、選択シナリオ／全件の週次キューと8868の別キューを確認。全体のバッチ・キャンペーン進捗は全件表示だけに限定し、controller SHAと異なる旧キャンペーンJSONを隠す。ジョブ操作・計算側設定・release Gitは変更していない。
- 修論用 `main` には `0ef5f301` と全件表示だけでバッチ進捗を集計する `7441f417` を統合し、フロント48テストと本番ビルドを確認した。求解・Prepared・費用・研究採用契約は変更していない。独立レビューと現SHAの正式研究実行は別ゲートであり、画面の動作確認を研究採用とは扱わない。

## 2026-09-23 GitHub向けREADMEの再構築と開発進捗の反映

- 目的: リポジトリのGitHubトップ画面における可読性・視認性・構成の抜本的改善。直近の開発進捗（渋24翌朝オーバーナイト診断、18台分散計算クラスタ、SOC夜間選択肢と誤計算防止ガード `NEXT_MORNING_SOC_NOT_READY`、BESS連続2週間診断）を、初見の教員・共同研究者・外部開発者でも直感的に理解できるように構成・反映した。
- 構成の改編:
  - ヒーローセクション（バッジ、システム概要、クイックリンク）を先頭に新設。
  - 目次（アンカーリンク付き）とMermaidによる全体処理フロー（アーキテクチャ図）を追加。
  - まえがき・先生向け要約、主要機能一覧、最新開発状況（2026-09-23要約）、最短起動手順、最適化実行手順、結果の読み方、分散クラスタ運用、厳格な研究契約・ガードレール、トラブルシューティング、関連資料テーブル、リポジトリ見取り図の順に論理的に再編。
  - 過去の実験・診断ログ（8月分等）は末尾の「過去の実験・診断アーカイブ」へ統合・保全し、既存の比較ハッシュや見出しアンカーを破壊しないよう配慮。
- リンク・テスト検証:
  - `tests/test_readme_navigation.py`（必須キーワード・ローカルリンク解決・見出しアンカー解決）および `tests/test_simplified_layout.py`、`tests/test_repository_layout.py` を実行し、全15件が通過することを確認。
...
## 2026-09-24 Solcast学習履歴の日次取得

- 当日の初回確認で既存32/36か月の月次マニフェスト、地点、PT15M、7項目の有限値、月内連続性、request/raw SHAを再検証。最初の不足月2023年9月のみを選び、事前のdry-runとPowerShell構文検査を通過した。
- 現ユーザーDPAPI認証を子プロセスのSOLCAST_API_KEYにだけ渡し、Solcast既存履歴取得経路で1リクエスト。残量照会は未確認で追加取得なし、契約変更・課金アップグレードなし。
- 2,880件を追加検証し、33/36か月・96,384件。以前の原本hashは不変で、残りは2023年10〜12月。`training_history_acquisition_status.json` のlast_checkと通知記録、`output/seven_day_extension_20260910/solcast_heartbeat_20260924.json` を更新した。
- 全36か月未完了のため新しい3年学習モデルは未生成。既存2024年学習、凍結済み月別比較、月別計算、監査、旧失敗処理、完了メールには触れていない。追加監視・エージェント起動なし。
...
## 2026-09-24 週次成果回収と三路線四季計算への切替

- 実行コードは隔離ブランチ `codex/weekly-seasonal-results-20260924` のclean固定 `0964b783c150173295c424a28ab9d5111dcddd7a`。この作業ツリーの既存変更は保持した。
- 旧7cb46894の12週について原本SHA・物理監査・1704便/週・確定rolling会計を照合し、週次/84日CSV、全便割当・車両SOC、12枚の7日図、季節図と日本語考察を `output/weekly_seasonal_20260924/results` へ保存。旧研究判定は保持し、条件付き週次評価だけを追加。3月4,873,779.232267円、7月4,153,640.193807円、差720,139.038459円。差の大部分は3月の買電207,193.774833円と契約超過507,913.787969円。同じ車両使用費4,120,000円である。
- 渋21 v4の実失敗は共有120秒に対しモデル構築233.8195秒で予算を使い切ったもの。メモリ停止ではない。新版はStage1 1800/Stage2 600/共有7200秒、4threads、既存barrier no-crossover、完全帰庫接続因子と端点充電表現。BFFのPrepared制御引継ぎを補正。採用prefixの確定plan・日別台帳を会計と同じオブジェクトから保存し、旧benchmark専用ファイル名参照を解消。関連99件通過。回収処理の原データfixture再集計と不足夜間区間拒否も通過。
- b02e0375は初回Prepare途中・worker投入0件で停止して証拠保持。0964b783の親機/32GB機のGit・runtime・データ一致を確認後、22:43 JSTに `campaign_0964b783` を起動した。まず2025-05-12、その後11-10、02-03、08-04。まだ新版7日間の完走結果ではない。
- DESKTOP-6AE0MIRは管理下の実license test通過。64GB DESKTOP-3PRU7QPはEnv起動失敗で求解へ割り当てない。資格情報は配布・変更していない。親機も週次求解を担当し、最低空きRAM18GB＋OS等予約2GBをschedulerで確認する。
- 全PC監視8868の既存設定には計算用外部2枠予約を明示。旧診断4コントローラーは全試行の終端を照合後に停止。旧8888はジョブ0件を確認し、既存drain APIで両workerの新規割当を停止。新版8890だけが2枠を消化する。停止・再起動要求の一部は自動承認審査に拒否されたため、この安全な受付停止を使用した。
- 0964b783用terminal observerの起動は自動承認審査が `blocked by policy` として拒否。計算スクリプト自体は起動済みだが、今回の終端メール通知は未有効。メール送信済みと表示しない。通常監視にAIを呼ぶ代替は作っていない。
- 二シナリオの週次化はユーザーへ対応関係だけ確認中。高PV/低PVの単日参照を同じ実PV週へ置き換えて同一ケースを2件と数えず、確認済みの基準構成を先行する。

## 2026-09-24 23:14 JST: 既存求解を維持したAI非依存運転入口

- operator独立ブランチ codex/weekly-operator-20260924 / 3c3f066c。計算release0964b783はcleanのまま。
- 実障害は5月batch-state保存のWinError5。workerの求解は継続。同一attemptをGETして確認し、新規投入なし。
- atomic_file.replace_bytesをbatch/週次記録へ導入（今後の版向け）。読取共有ロック解除待ち、原本保持、7回限定。
- 69件PASS。Windows実機の共有ロックを再現。公開SSH故障ケースはmock、稼働中PCを故意に切断していない。
- 操作入口6本＋日本語手順、status JSON/Markdown、既存attempt回収、未送信EML生成を実装。
- AIもCodex queueも呼ばないwatchをHidden起動、PID49876。実ログ更新・status並行使用・再開二重起動拒否を確認。
  以前のCodex通知observer起動拒否を再実行したものではなく、外部送信もAI復帰もしないローカル回収処理。
- この時点で4/4入力準備、5月1件求解中、3件QUEUED。親機は予約控除後16.37GB/要求18GBで待機。
  要件・SOC・目的・比較条件・ライセンス枠の変更なし。
- 証拠 output/weekly_seasonal_20260924/operator/verification.json。単独メール認証は未設定。
- 自己点検: 保存拒否のP1を修正。既存attempt以外のsubmitなし、original監査保持、unknown非失敗化、未送信明記を確認。
  独立レビューは未了。新版7日完走・全18台の大規模求解可否・PC再ログオン運転・実SSH故障注入は未確認。
...
## 2026-09-25: 路線カタログの段階表示と距離点検

Reactの対象範囲と路線画面に営業所→系統→パターン→停留所列の読取表示を追加。ODPT全域764件を主表示、GTFS全域618件を参考表示、シナリオ内を編集対象とし、既存の路線編集APIと最適化入力は変更しない。表示用の距離は起終点係数・停留所数補完から、既存の最適化準備と同じ隣接停留所Haversine積算へ変更。座標欠損は未算出にする。数理制約、費用、研究受入条件への変更なし。

対象範囲は営業所にチェックを入れるまで系統・パターンを展開せず、チェック時にその営業所の所属パターンを対象範囲へ追加する。営業所のチェック解除時は残る営業所に属さないパターンを対象範囲から除く。閲覧画面では参考所管も確認できない路線を「所管未確認」で見せる。各パターンには停留所順・座標、日区分別便数・始発・最終到着、表示距離と保存距離を区別して示す。

ODPT全域の原本は `data/catalog-fast/tokyu_bus_data/routes.jsonl`（SHA-256 `b5b790c75ef23486ae1b818cadce2ec3e1416cc0a044d4ec6867f85be72726e2`、元snapshot ID `20260311T044200Z`）。GTFS参考原本は `data/tokyu_gtfs.sqlite`（SHA-256 `a0e4bf1ce0a562454fdb3401e0793fe05e7d4c992235ac44bd6fa9635bc57a87`）。両者の収録件数と分類は異なり、現行運行の完全な名簿とは断定しない。画面は保存済みファイルを読取専用で参照し、ODPT/APIへの自動再取得は行わない。

シナリオ内停留所が部分保存の場合は、IDが一致するODPT停留所の保存済みカタログ座標を表示専用で補助参照し、距離・各停留所の出典を明記する。正式入力には反映しない。既存の全域シナリオ1件では764路線の保存距離が全件0、座標補助後の表示距離は全件算出できた。ゼロの正式入力問題は未解決として明示する。

渋21～23以外はODPT 746パターン・32,706便と、別GTFS 603パターンを点検し、結果を `docs/notes/ROUTE_CATALOG_AUDIT_20260925.md` に記録。ODPTは保存所管未割当254件、路線距離0が746件、便別距離0が32,706件、収録便0が7パターン。森02・蒲01・黒01の3パターンでは計42便の終点が登録パターン終点と不一致。便数と日区分別便数は原本 `tripFile` と一致した。保存済みの全764件が本線の手動分類で、区間便等の分類は未検証。画面では保存分類・距離0・収録便0を明示し、終点不一致は点検記録に残す。便別距離や便とパターンの対応は自動修復しない。独立レビューと便別の公式資料全件照合は未了。

検証: `.venv/Scripts/python.exe -m pytest tests/test_route_catalog_browser.py tests/test_route_catalog_audit.py tests/test_desktop_editing.py -q` は32件PASS、`frontend` の `npm.cmd test` は28件PASS、`npm.cmd run typecheck` と `npm.cmd run build` はPASS。Electron smoke は稼働中controllerのロックと競合したため、`MC_CLUSTER_DIR` を独立した出力へ指定して再実行しPASS（全10画面訪問、認証済みBFF、`output/desktop-smoke/smoke.json`）。画面4でODPT全域を初期表示し、営業所選択前にはパターンを表示しないことも画像で確認。最適化・Prepare・研究採用の検証ではない。

### 公式営業所資料の追加照合（同日）

agent-browserの専用セッションで東急バス公式の営業所別PDF一覧、淡島のPDF、およびバスナビ路線一覧を操作した。リンクを確認して12営業所の現在配信PDFを取得し、画像の表題・日付・系統表を目視した。検索エンジンが同じPDF URLに対して旧版の表を返す例があったため、実際に配信されている画像を優先した。PDFの日付は2023-04-01から2026-07-01まで幅があり、新羽などは主要系統のみ記載。`data/seed/tokyu/depots.json` の路線図日付も淡島・荏原・新羽では現在配信版より古い。この閲覧から保存シードや研究入力は更新しない。

ODPT保存所管未割当254パターンのうち、系統コードと公式図面／営業所明記のお知らせを照合できた31コード・79パターンに、`data/reference/tokyu_route_depot_reference_20260925.json` から表示専用の `officialDepotReference` を付ける。参照は元ODPTの `sourceSnapshotId` に結び付け、IDが変われば参考所管を出さず警告する。参照の取得日と資料日付を画面に出し、`depotIds` は空のまま保持する。残り175パターンは「所管未確認」で閲覧可能。空港・高速・直行・出入庫等の共通コードは系統番号だけで割り当てない。梶01は高津の2025年12月PDFと虹が丘の2026年8月案内の両方にあり、単一所管に確定せず資料不一致を画面に表示する。新横81はODPTスナップショット後の2026-03-31に運行終了したという公式発表を画面に表示する。両件の出典と残件内訳は `docs/notes/ROUTE_CATALOG_AUDIT_20260925.md` に記載した。ODPT原本、シナリオ、Prepare、最適化、研究判定への変更はない。

回帰確認: 追加した表示参照の非変異・スナップショット不一致テストを含むPython対象3ファイル33件PASS、フロント全29件PASS、TypeScript typecheckと本番build PASS。独立レビューと便別所管の完全照合は未了。
