...
- キュー内の2枠・通信不明保持・330秒解放待ちを維持し、別ポート/別queueで二重に枠を作る経路をauthority指定で拒否。ライセンス失敗workerは検証済み終了後に自動割当停止。
- 今回の4台はWindows/SSH読取で32/32/32/64GBと実測。64GB機は既知ライセンス失敗でdisabled。旧8890の未完了/待機0を確認して停止し、再起動してもexternal=2に固定。8868 external=2、8891 external=0/使用2を照合。稼働中の固定cf4beb97やattemptは変更・再投入していない。
- Python124件、画面60件、型検査と本番build通過。現行環境の設定制限と次版の強制コードを区別。READMEとdocs/notes/GUROBI_ADMISSION_POLICY_20260925.mdに手順・根拠・未検証を記録。自己レビュー済み、独立レビュー未実施、アプリ外の同資格情報利用は外部予約が必要。

## 2026-09-25 計算途中の工程・ログ・待機理由を画面へ反映

- 期間別計画と分散計算に共通の詳細進捗を追加。入力準備、Stage 1/2、毎時計算の保存済み可行窓率、回収・検算、担当PC、ログ時刻・経過秒・反復・stage gap、待機理由、エラー、試行IDと固定SHAを確認できる。狭い画面で表が崩れる問題をカード化で修正。
- AI不要の読取publisherを追加し、稼働中のcf4beb97計算へソース変更せず適用。SSH stdin経由の読取、ID/hash照合、複数run混在の拒否、接続不明時の証拠保持、期限切れ表示、多重監視ロックを実装。CLIは複数operationに対応し、今回専用の路線/月をハードコードしない。
- 25 Python / 60 frontend testsと本番build通過。2台の実SSH読取と8868の実表示を確認。1月・3月の64GB機はGUROBI_LICENSE_UNAVAILABLEで失敗しており新規割当を停止、2月・4月の試行を継続。未解決のライセンス問題を成功・メモリ問題に読み替えない。
- READMEに操作と監視再開方法、docs/notes/EXECUTION_DETAIL_PROGRESS_20260925.mdに配置・数値の意味・試験・未実施事項を記録。正式mainへ同期する表示・運用機能の変更であり、実行固定版・数理条件・研究判定は変更しない。独立レビュー未実施。

## 2026-09-25 最適化用カタログ経路・距離とmain基準の修正

- SQLite便抽出とbuilt出力を便別停留所列の距離へ統一し、区間便/循環便・operator・元pattern ID・hashを保持。原本や凍結済みシナリオは変更しない。
- Prepareの欠損座標0扱い、無効距離受付、列不整合、HTTP500化、存在しないexport依存、Windows読取接続未closeとDB公開順序を修正。
- 全社33,484便、GTFS参考33,354便、旧ODPT33,360便を別々に確認。旧終点差42便は便自身の列と一致。最終built→Prepare距離再算出は33,484便で差0 km。
- 関連Python101件通過。自己レビュー済み。独立レビュー・新版7日間求解・実行中worker更新は未実施。
- 統合済み未使用12ブランチを復元可能なrefs/archive/20260925へ退避し整理。未統合・worktree・固定実験・PR用ブランチと原本は保持。
- 手順・証拠・限界: docs/notes/OPTIMIZER_INPUT_REPAIR_20260925.md。実装を現在ブランチ・main・週別計画ブランチの共通基準へ同期する。
...
## 2026-09-25 週次controllerへ分散監視画面を移植

- 稼働中の `0964b783` は旧 `906a4253` よりバッチ進捗・キャンペーン進捗・ジョブ工程率・期間表示が増えている。週次版を基点に、シナリオID絞込み、全件切替、ローカル別ポート閲覧、ポート別キャッシュ、HTTPキャッシュ無効、画面復帰・再接続時更新、監視先保持を `0ef5f301` で移植した。バッチ・キャンペーンの全体集計は全件表示だけで見せる。キャンペーンJSONは選択したcontrollerから読み、未配置またはcontroller SHAと異なる旧版なら欄を隠す。ジョブ工程率の既存表示は維持する。
- フロント47テストと本番ビルドを確認。8890の既存controller（PID 52180、計算用SHA `0964b783`、source digest `d66759b2`）を再起動せず、版別成果物 `output/cluster-deployment/frontend-weekly-ui-0ef5f301/dist` の静的JS/CSSとindexだけを配置した。旧indexのSHA-256先頭 `14e7ae4f` を保全し、新indexは `702b6a48`。HTTP配信と実ブラウザーで選択シナリオ、全件の週次キュー、8868の別キューと閲覧専用操作を確認した。旧渋24のキャンペーンJSONはcontroller SHA不一致で非表示。ジョブ操作・settings・計算release Gitは変更していない。
- 別controllerへの変更操作を無効化し、ジョブはそのcontroller自身で操作する。これは表示の動作確認であり、求解・Prepared・出典・研究採用契約の検証や研究採用ではない。

## 2026-09-25 修論用本線の分散バッチ表示

- `main` に `0ef5f301` の監視先・シナリオ切替と `7441f417` のバッチ進捗修正を統合した。バッチは宣言した全タスク数を分母とするため、個別シナリオに絞ったジョブを渡さず、全件表示時だけ全ジョブから集計する。これにより別シナリオの固定表示と、部分集合を全バッチ進捗として誤読する表示を防ぐ。フロント48テストと本番ビルドを確認した。求解・Prepared・費用・研究採用条件は変更していない。独立レビューと現SHAの正式研究実行は別ゲートであり、この画面確認だけで研究採用を宣言しない。
...
## 2026-09-24 分散計算画面のシナリオ・監視先切替

- 選択中シナリオのIDでキュー履歴を絞り、全件表示へ切り替えられるようにした。計算端末は監視先全体の情報として表示する。
- 同一PCの別controllerは監視先ポートを指定して閲覧する。8868の旧キューと8890の稼働キューを混同しないよう接続先と最終更新時刻を示す。別監視先ではジョブ・端末操作を無効化する。
- ローカル別ポートだけにCSP通信を許可する。ジョブと端末のポーリングはHTTPキャッシュを使わず、画面復帰・再接続時にも取得する。求解・Prepared・研究採用契約は変更しない。
- 監視先ポートは同じブラウザタブ内で保持し、シナリオ切替や再読込でも同じキューを見られるようにした。別監視先ではシナリオIDが異なることがあるため、全ジョブ表示から始める。
- clean commit `785ab01d` からビルドしたフロント3ファイルを `output/cluster-deployment/frontend-cluster-ui-785ab01d/dist` にハッシュ照合して配置。8868は実行中ジョブ0件を確認後、frontendパスだけ変更して再起動した。8890は実行中ジョブがあるため、その配信ファイル・設定・プロセスには触れていない。8868のブラウザで8890のRUNNING/QUEUEDジョブ、別監視先の操作無効、選択中シナリオの履歴絞込みと全件切替を確認した。
...
- 別チャットで配置された011c0bd6の監視先切替・シナリオ絞込み・更新停止警告を統合。各シナリオの期間別計画へ集約。旧campaign進捗は全件表示かつ接続先SHAが一致する場合だけの互換表示とする。フロント全47件通過。

## 2026-09-25 期間管理のmain統合・配置確認

- 並行作業の0ef5f301とmainの7441f417を内容照合して統合。個別シナリオでは全バッチの分母を表示せず、全件表示のみで集計。監視先変更時に他controllerのcampaignを混ぜない。
...
## 2026-09-25 Solcast履歴日次確認：2023年10月を追加

- 前回確認は2026-09-24 JST。月次マニフェスト、要求・原本SHA、地点、PT15M、7項目の有限値、月内連続性を再検証し、取得済み33か月・96,384件と不足2023年10〜12月を確定した。
- 現ユーザーのDPAPI暗号化認証を子プロセスの環境変数だけに渡し、既存 `scripts/weather/acquire_solcast_history.py` で2023-10-01〜2023-11-01未満を1回だけ取得した。残量照会は未確認。契約変更・購入・追加リクエストなし。
- 10月2,976件を検証し、累計34/36か月・99,360件へ更新。原本SHA-256は `c0a5c83f0da30f475c9bcc5e246e1b226b5055c4a451fe574ab80b061dab94ef`。既存33か月の原本SHAは不変。残りは2023年11月・12月。
- 実行記録は `output/seven_day_extension_20260910/solcast_preflight_20260925.json`、`solcast_request_20260925.json`、`solcast_heartbeat_20260925.json` と同ディレクトリの取得状況JSONに保存した。検証は実データ34か月に対してPASS。取得・検証の本体コード変更はなく、日付と対象月だけを既存の日次ラッパーに設定した。
- 3年分の派生データ・新学習モデルは全36か月検証後の別版とする。既存2024年学習モデル、凍結済み比較入力、計算ジョブ、完了メール、監視構成は変更していない。

## 2026-09-25 新版の月別12週実行に向けた切替（求解開始前）

- ユーザー指示：統合済み新版で「仮・正式用」の各月1週・計12週を実行し、障害を修正してmainへ反映。対象は渋21〜23、2025年の既定12週。四季4週への削減はしない。
- 出発点8efae778：他チャットの統合、翌朝SOCと週末延長、Stage1 seedの再試行保存、ODPT原本/最適化DB分離、trip距離監査・SQLite読取接続close等の修正済み版。旧0964b783の出力は新版へ再ラベルしない。
- 開始前の実測：8890旧版はFAILED6件/QUEUED6件、実行中・不明0件、予約/冷却0件。8868管理側も実行中0件、共有2枠を8890用に外部予約。旧失敗はseed保存のFileExistsError等であり、新版で修正済み。旧未投入は記録保持のままcancelし、新旧の二重投入を防ぐ。
- 障害P1：週を親機へ交互に固定していたため、親機の空き約14GB<要求18GBでも6週が待機し、64GB機の約47GBを使えない。新しい `--workers auto` はworker指定をnullにし、既存schedulerの資源・版・ライセンス審査を再利用。autoと固定指定の混在はPrepare前に拒否。旧operationの固定指定は変更しない。
- 配置候補は親機、64GB DESKTOP-3PRU7QP、32GB DESKTOP-6AE0MIR/LAPTOP-A709UNA0。固定コード・環境・データを照合してから候補に入れる。共有Gurobi2枠と終了後待機を維持し、親機のOS/アプリ余裕を残す。軽量監視ではEnvを起動しない。
- 全週で既定seed42/4threads、Stage1 1800秒/Stage2 600秒/共有7200秒、15分刻み・毎時rollingを維持。BEV翌朝運用上限到達、最終翌朝の充電・受電・費用を含む。BESS補助運用/追加予備なし/終端復元なしを維持。ODPT再取得・DB再構築なし。
- 実行・検算・会計・最適性・研究採用は別。求解時間上限内の可行解は検算後に週次評価へ進めるが統合大域最適としない。正式研究採用/独立レビュー完了は宣言しない。
- 通常の監視・回収・図表化は既存Python/PowerShellで実行。終端通知の表題を四季から月別代表週へ修正。配置・起動・試験の証拠はoutput/monthly_latest_20260925へ追記する。
...
## 2026-09-26 17:39 JST — メモリ修正の実機再検証と読み取り専用計測

- 旧 a06a4560 の試行9da36eb7を利用者指示による新版適用のため取消。remote PID10756の不在とCANCELLEDを17:26に確認。意図した取消を障害メールとして送信していない。
- 0b433705のclean固定版を5台へ配置・照合し、8891コントローラーを同版へ変更。2月代表週を新規Prepareし、17:39時点は投入済み（求解完了ではない）。32GB機のtask16GiB/native hard14GiB・共有Gurobi2枠と330秒解放待ちは維持。
- 計算プロセスの実RAM/最大RAM/専用コミットを読取側へ追加。Windowsのquery-only handleで生成時刻を照合してからK32GetProcessMemoryInfoを呼ぶ。32bit readerは未対応と表示し、PID再利用・照会拒否を0へ変換しない。履歴は親機のattempt operations内のみ、固定workerソース・計画・物理条件は変更しない。
- Python対象10件（Windowsの自己プロセス実照会/PID生成時刻不一致を含む）、フロント8件、TypeScriptを含む本番ビルド通過。既存stale表示テストは新メモリ表示との複数一致を限定したselectorへ修正して再通過。GurobiのEnv/求解はこの計測テストで起動しない。
- 既存認証済みClaude Code2.1.283へ利用者の明示指示に従いOpus5.5を指定。実応答model claude-opus-5-5を確認。秘密情報を含まないソースコピーに限定したRead/Grep/Globのみの独立レビューを依頼中。結果は未受領で、独立承認済みとはしない。
- 全体プロセスが16GiB内で前回失敗窓を通過するかは実機計算で継続確認する。コードの部分保持量削減をプロセス全体の削減量として報告しない。
