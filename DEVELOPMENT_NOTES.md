...
- 稼働中の `0964b783` は旧 `906a4253` よりバッチ進捗・キャンペーン進捗・ジョブ工程率・期間表示が増えている。専用ブランチの旧ビルドはそのまま使わず、週次版を基点にシナリオID絞込み、全件切替、ローカル別ポート閲覧、port別キャッシュ、HTTPキャッシュ無効、再接続時更新、監視先保持を移植する。バッチ・キャンペーンの全体集計は全件表示でのみ見せ、選択シナリオに別ケースの固定表示や不正な分母を出さない。キャンペーンJSONは選択したcontrollerから読み、未配置またはcontroller SHAと異なる旧版なら欄を隠す。ジョブ工程率の既存表示は維持する。
- 別controllerへの変更操作を無効化し、ジョブはそのcontroller自身で操作する。画面は求解・Prepared・出典・研究採用契約を変更しない。進行中の週次計算を止めず、静的UIだけを版別に検証して配置する。

## 2026-09-25 修論用本線の分散バッチ表示

- 既存のシナリオID絞込みと別controller監視を維持する。バッチは宣言した全タスク数を分母とするため、個別シナリオに絞ったジョブを渡さず、全件表示時だけ全ジョブから集計する。これにより別シナリオの固定表示と、部分集合を全バッチ進捗として誤読する表示を防ぐ。求解・Prepared・費用・研究採用条件は変更しない。
