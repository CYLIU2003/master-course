...
# -*- coding: utf-8 -*-
"""
Test optimization using prepared input for scenario 237d5623-aa94-4f72-9da1-17b9070264be
This uses the prepared input JSON which contains all trips/vehicles/etc.
"""
import json
import sys
import time
...
from bff.mappers.scenario_to_problemdata import build_problem_data_from_prepared_input
...
PREPARED_INPUT_ID = "prepared-a04f296d42df0e60"

def load_prepared_input():
    """Load prepared input JSON."""
    prepared_path = Path(f"output/prepared_inputs/{SCENARIO_ID}/{PREPARED_INPUT_ID}.json")
    with open(prepared_path, "r", encoding="utf-8") as f:
        return json.load(f)

def load_scenario():
    """Load scenario document for overlay config."""
...
def run_optimization_mode(mode_name: str, mode: OptimizationMode, time_limit: int = 60):
    ...
    try:
        ...
        print(f"[OK] Loaded prepared input")
        print(f"   - Trips: {len(prepared.get('trips', []))}")
        print(f"   - Vehicles: {len(prepared.get('vehicles', []))}")
        print(f"   - Chargers: {len(prepared.get('chargers', []))}")
        
        # Load scenario for overlay (PV, pricing, etc.)
        scenario = load_scenario()
        print(f"[OK] Loaded scenario overlay")
        
        # Merge scenario overlay into prepared input
        if 'simulation_config' in scenario:
            prepared['simulation_config'] = scenario['simulation_config']
        if 'scenario_overlay' in scenario:
            prepared['scenario_overlay'] = scenario['scenario_overlay']
        
        # Build problem from prepared input
        builder = ProblemBuilder()
        problem = builder.build_from_prepared_input(
            prepared,
            depot_id="tsurumaki",
            service_id="WEEKDAY",
        )
        ...
        for depot_id, asset in problem.depot_energy_assets.items():
            pv_total = sum(asset.pv_generation_kwh_by_slot) if asset.pv_generation_kwh_by_slot else 0
        ...
        # Print cost breakdown (only non-zero values)
        ...
        # Check key metrics
        print(f"\n[METRICS] Key Metrics:")
        print(f"   - Served trips: {len(result.plan.served_trip_ids)}/{len(problem.trips)}")
        ...
        if pv_gen > 0:
            ...
            elif pv_utilization < 20:
                issues.append(f"[WARN] Low PV utilization: {pv_utilization:.1f}%")
            elif pv_curt > pv_gen * 0.8:
                issues.append(f"[WARN] High PV curtailment: {pv_curt/pv_gen*100:.1f}%")
        
        coverage = len(result.plan.served_trip_ids) / max(len(problem.trips), 1) * 100
        print(f"   [INFO] Trip coverage: {coverage:.1f}%")
        
        if coverage < 100:
            issues.append(f"[WARN] Incomplete coverage: {coverage:.1f}%")
...
def main():
    ...
    print(f"{'#'*80}")
    print(f"# Optimization Test Suite - Prepared Input Method")
    ...
    print(f"\n{'Mode':<8} {'Objective':>15} {'Vehicle':>12} {'Energy':>12} {'PV Used':>10} {'Coverage':>10} {'Status':<12}")
    print(f"{'-'*92}")
    ...
    for mode_name in ["MILP", "ALNS", "ABC", "GA"]:
        ...
        if result:
            ...
            cov = len(result.plan.served_trip_ids)
            status = "[OK]" if result.feasible else "[FAIL]"
            print(f"{mode_name:<8} {obj:>15,.0f} {veh:>12,.0f} {eng:>12,.0f} {pv:>10.1f} {cov:>10} {status:<12}")
        else:
            print(f"{mode_name:<8} {'ERROR':>15} {'-':>12} {'-':>12} {'-':>10} {'-':>10} {'[ERROR]':<12}")
    
    print(f"\n[DONE] Test suite completed!")
    
    # Save results summary
    summary_path = Path("output/tmp_optimization_test_237d_summary.json")
    summary = {
    ...
    for mode_name, result in results.items():
        if result:
            summary["results"][mode_name] = {
                ...
                "served_trips": len(result.plan.served_trip_ids),
                "unserved_trips": len(result.plan.unserved_trip_ids),
            }
