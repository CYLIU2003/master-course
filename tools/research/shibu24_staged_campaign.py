"""Run the frozen Shibu24 diagnostic in day, week, then twelve-month gates.
...
def verify_prepared(path: Path, expected_trips: int) -> None:
    ...
    if any(not trip.get("operator_id") or str(trip["operator_id"]).upper() == "UNKNOWN"
           ...
           for trip in trips):
        raise ValueError("Prepared Shibu24 trips lack operator or positive proven distance")
...
def prepare_day(directory: Path) -> dict:
    ...
    scenario_id = scenario_store.duplicate_scenario(
        monthly.PARENT_ID, name=f"渋24 {DAY} 実便1日・翌朝SOC段階診断"
    )["id"]
    ...
    verify_prepared(prepared.solver_input_path, 224)
    state = {"scenario_id": scenario_id, "prepared_input_id": prepared.prepared_input_id,
             ...
             "trip_count": 224, "overnight": overnight, "parent_sha256": original_hash}
    write(path, state)
    return state
...
def run(settings_path: Path, directory: Path, minimum_ram_gb: float) -> dict:
    ...
    try:
        if state["stages"].get("day") != "PASSED":
            ...
            write(state_path, state)
            run_batch(one_task_spec(settings, task, "shibu24-stage-day-v1"), directory / "day")
        ...
        if state["stages"].get("months") != "PASSED":
            ...
            remaining["batch_id"] = "shibu24-stage-remaining-eleven-v1"
