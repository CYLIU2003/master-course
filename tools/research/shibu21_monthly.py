"""Audit and prepare twelve real Shibu21 weeks with paid final overnight slots.
...
def _source_and_design() -> tuple[dict, dict, list[str]]:
    database_manifest, tables = load_database(DATABASE_DIR)
    if (database_manifest.get("source_status") !=
            "DIAGNOSTIC_SOURCE_CAPTURE_VALIDATED_DISTANCE_PROXY_DECLARED" or
            database_manifest.get("source_scope") !=
            {"route_codes": ["渋21"], "route_count": 6, "template_count": 72}):
        raise ValueError("Unexpected frozen Shibu21 source audit or route scope")
    if any(unicodedata.normalize("NFKC", str(row.get("routeCode") or "")) != "渋21"
           for name in ("selected_routes", "timetable_rows") for row in tables[name]):
        raise ValueError("Optimization database includes a non-Shibu21 route")
    forecast_manifest = _read(FORECAST / "manifest.json")
    design = deepcopy(forecast_manifest["design"])
    if design.get("route_codes") != ["渋21", "渋22", "渋23"]:
        raise ValueError("Forecast holdout design is not the declared three-route parent")
    # Reuse only its frozen calendar/weather/asset controls. This campaign's
    # timetable scope is explicitly narrowed before any scenario is prepared.
    design["route_codes"] = ["渋21"]
    design["route_scope_mode"] = "single_route_case"
    design["scope_change_reason"] = "Shibu21-only staged diagnostic after Shibu24 memory failure"
    ...
    source = {
        ...
        "source_id": "tsurumaki_shibu21_odpt_subset_20260924_diagnostic_v1",
        "route_codes": ["渋21"],
...
def check() -> dict:
    ...
    return {"schema_version": "shibu21_monthly_preflight_v1", "status": "INPUTS_AVAILABLE_DIAGNOSTIC",
            "year": 2025, "source_manifest_sha256": source["source_manifest_sha256"],
            "route_scope": ["渋21"], "source_template_count": len(templates),
            ...
            "route_design": {"route_codes": design["route_codes"],
                             "route_scope_mode": design["route_scope_mode"]},
    ...
    for week in weeks[:limit]:
        ...
        if not state.get("scenario_id"):
            scenario_id = scenario_store.duplicate_scenario(
                PARENT_ID, name=f"渋21 2025年{int(week[5:7])}月代表週 {week} 翌朝SOC診断"
            )["id"]
        ...
        if state["status"] != "PREPARED":
            break
    return {"schema_version": "shibu21_monthly_prepare_v1", "binding": binding,
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "output/shibu21_monthly_20260923")
    ...
    parser.add_argument("--batch-id", default="shibu21-monthly-overnight-v1")
