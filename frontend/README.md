...
「対象範囲」では営業所を選んでから所管系統、運行パターンを順に開きます。路線画面は「ODPT全域」で保存済み764パターンを主表示し、「GTFS参考」で別スナップショットの618パターンを、「このシナリオの路線・編集」で当該シナリオを表示します。停留所順、日区分別便数、隣接停留所の直線距離和を確認できます。ODPT保存所管が空の一部パターンは公式路線図などの参考所管で営業所別に表示し、出典・資料日付・保存所管未割当を明示します。参考所管も確認できないパターンは「所管未確認」に残します。シナリオ内の停留所座標が不足する場合は保存済みODPT停留所カタログを表示に限り補助参照し、その旨を表示します。ODPT全域には `data/catalog-fast/tokyu_bus_data/routes.jsonl` と正規化停留所、GTFS参考には `data/tokyu_gtfs.sqlite` が必要です。[点検記録](../docs/notes/ROUTE_CATALOG_AUDIT_20260925.md)。
