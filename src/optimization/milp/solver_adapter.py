...
def _stage1_candidate_evaluation_priority_key(
    candidate: Tuple[int, float, str, str, AssignmentPlan],
) -> Tuple[int, float, str, str, int]:
    """Prioritize the weather-aware Stage 1 lower-cost candidates.

    Stage 2 remains the canonical physical-cost authority.  This ordering only
    determines which candidates receive scarce Stage 2 wall time first; it
    does not change the candidate set, add a BEV preference, or claim global
    optimality from the Stage 1 relaxation.
    """

    pool_index, raw_objective, candidate_hash, candidate_source, _plan = (
        candidate
    )
    objective = float(raw_objective)
    objective_is_invalid = int(not math.isfinite(objective))
    sortable_objective = objective if math.isfinite(objective) else math.inf
    return (
        objective_is_invalid,
        sortable_objective,
        str(candidate_hash),
