...
def _candidate_gurobi_homes() -> list[Path]:
...
def _configure_gurobi_runtime() -> None:
    ...
    for candidate in _candidate_gurobi_homes():
...
def _ensure_gurobi():
    """Re-try gurobipy import at solve-time.

    The module-level import may have failed in a frozen (PyInstaller) environment
    where DLL search paths are set up only after module load.  This function
    retries so that a fresh invocation (after paths are configured) succeeds.
    Returns (gp_module, GRB_module) or raises RuntimeError.
    """
    global _GUROBI_AVAILABLE, gp, GRB  # noqa: PLW0603
    if _GUROBI_AVAILABLE:
        return gp, GRB
    _configure_gurobi_runtime()
    try:
