...
_DRAG_CAPTURE_JS = """(function () {
  ...
  function bindDragHandlers() {
    var map = resolveMap();
    if (!map) { setTimeout(bindDragHandlers, 250); return; }
    map.eachLayer(function (layer) {
      ...
      if (title.indexOf('__stop__') !== 0 && title.indexOf('__depot__') !== 0) return;
      ...
      layer.on('dragend', function (evt) {
        ...
        window.__GLOBAL_DATA__.lat_lng_clicked = {
          ...
          lng: ll.lng,
          _action: 'move_depot',
          depot_id: title.slice('__depot__'.length)
        };
      });
    });
  }
...
def _generate_segments(
    stops_df: pd.DataFrame, route_id: str = "route_101"
) -> pd.DataFrame:
...
def _load_all(data_dir: str) -> dict:
    ...
    if stops_df is not None:
        ...
        if (
            stops_df["distance_from_prev_km"].isna().all()
            or (stops_df["distance_from_prev_km"] == 0).all()
        ):
...
def _render_stats(stops_df: pd.DataFrame, depots_df: pd.DataFrame) -> None:
    ...
    for direction in stops_df["direction"].unique():
        ...
        with cols[4]:
            off_route = (
                len(
                    depots_df[depots_df.get("on_route", pd.Series(dtype=bool)) == False]
                )
                if "on_route" in depots_df.columns
                else 0
            )
...
def _render_simple_editor(data: dict, data_dir: str) -> None:
    ...
    for c in hidden_cols:
        if c not in edited_stops.columns:
            edited_stops[c] = (
                stops_df[c].values[: len(edited_stops)]
                if len(stops_df) >= len(edited_stops)
                else ""
            )
    ...
    with st.form(key=_sk("quick_add_stop"), clear_on_submit=True):
        ...
        with qc[0]:
            q_seq = st.number_input(
                "順序", min_value=1, value=len(edited_stops) + 1, step=1
            )
        with qc[1]:
    ...
    st.caption(
        "バス停リストからセグメントが自動生成されます。保存時に segments.csv に反映されます。"
    )
...
def _render_map_editor(data: dict, data_dir: str) -> None:
    ...
    pending_stop_key = _sk("pending_stop_moves")
    pending_depot_key = _sk("pending_depot_moves")
    if pending_stop_key not in st.session_state:
        st.session_state[pending_stop_key] = {}
    if pending_depot_key not in st.session_state:
        st.session_state[pending_depot_key] = {}

    pending_stop_moves: dict[str, dict] = (
        st.session_state.get(pending_stop_key, {}) or {}
    )
    pending_depot_moves: dict[str, dict] = (
        st.session_state.get(pending_depot_key, {}) or {}
    )

    stops_view = stops_df.copy()
    if pending_stop_moves and len(stops_view) > 0 and "stop_id" in stops_view.columns:
        for stop_id, move in pending_stop_moves.items():
            m = stops_view["stop_id"] == stop_id
            if m.any():
                stops_view.loc[m, "lat"] = float(move["lat"])
                stops_view.loc[m, "lon"] = float(move["lon"])
        if "direction" in stops_view.columns and "sequence" in stops_view.columns:
            stops_view = _compute_distance_from_prev(
                stops_view.sort_values(["direction", "sequence"]).reset_index(drop=True)
            )

    depots_view = depots_df.copy()
    if (
        pending_depot_moves
        and len(depots_view) > 0
        and "depot_id" in depots_view.columns
    ):
        for depot_id, move in pending_depot_moves.items():
            m = depots_view["depot_id"] == depot_id
            if m.any():
                depots_view.loc[m, "lat"] = float(move["lat"])
                depots_view.loc[m, "lon"] = float(move["lon"])
    ...
    with st.expander("📊 路線統計", expanded=False):
        _render_stats(stops_view, depots_view)

    # --- 地図の中心を決定 ---
    center_lat, center_lon = _DEFAULT_CENTER
    all_lats, all_lons = [], []
    for df in [stops_view, depots_view]:
        if (
            df is not None
            and len(df) > 0
    ...
    m.get_root().script.add_child(BrancaElement(f"<script>{_DRAG_CAPTURE_JS}</script>"))
    ...
    for _, row in stops_view.iterrows():
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                ...
                html=(
                    '<div style="'
                    f"width:{14 if is_dep else 10}px;height:{14 if is_dep else 10}px;"
                    "border-radius:50%;"
                    f"background:{color};"
                    "border:2px solid #ffffffcc;"
                    'box-shadow:0 0 2px rgba(0,0,0,.35);"></div>'
                ),
            ),
            popup=folium.Popup(
    ...
    for direction in stops_view["direction"].unique():
        sub = stops_view[stops_view["direction"] == direction].sort_values("sequence")
        coords = []
    ...
    for _, row in depots_view.iterrows():
        lat, lon = row.get("lat"), row.get("lon")
        if pd.isna(lat) or pd.isna(lon) or lat == 0 or lon == 0:
            continue
        ...
        folium.Marker(
            location=[lat, lon],
            draggable=True,
            title=f"__depot__{row.get('depot_id', '')}",
            icon=folium.Icon(
    ...
    if map_result and isinstance(map_result, dict):
        ...
        if last_clicked and isinstance(last_clicked, dict):
            action = str(last_clicked.get("_action", "") or "")
            if action == "move_stop":
                moved_stop_id = str(last_clicked.get("stop_id", "") or "")
                new_lat = last_clicked.get("lat")
                new_lng = last_clicked.get("lng")
                if moved_stop_id and new_lat is not None and new_lng is not None:
                    pending = dict(st.session_state.get(pending_stop_key, {}) or {})
                    pending[moved_stop_id] = {
                        "lat": float(new_lat),
                        "lon": float(new_lng),
                    }
                    st.session_state[pending_stop_key] = pending
                    st.toast(f"一時移動: {moved_stop_id} (未適用)", icon="📍")
                    st.rerun()
            if action == "move_depot":
                moved_depot_id = str(last_clicked.get("depot_id", "") or "")
                new_lat = last_clicked.get("lat")
                new_lng = last_clicked.get("lng")
                if moved_depot_id and new_lat is not None and new_lng is not None:
                    pending = dict(st.session_state.get(pending_depot_key, {}) or {})
                    pending[moved_depot_id] = {
                        "lat": float(new_lat),
                        "lon": float(new_lng),
                    }
                    st.session_state[pending_depot_key] = pending
                    st.toast(f"一時移動: {moved_depot_id} (未適用)", icon="📍")
                    st.rerun()
            new_lat = last_clicked.get("lat")
            new_lng = last_clicked.get("lng")
    ...
    pending_stop_now = st.session_state.get(pending_stop_key, {}) or {}
    pending_depot_now = st.session_state.get(pending_depot_key, {}) or {}
    pending_total = len(pending_stop_now) + len(pending_depot_now)
    if pending_total > 0:
        st.info(
            f"ドラッグ移動の未適用変更: {pending_total} 件 "
            f"(バス停: {len(pending_stop_now)} / 車庫: {len(pending_depot_now)})"
        )
        pc1, pc2 = st.columns(2)
        with pc1:
            if st.button("✅ 地図の移動変更を適用", key=_sk("apply_map_drag_moves")):
                if pending_stop_now and "stop_id" in stops_df.columns:
                    for stop_id, move in pending_stop_now.items():
                        m = stops_df["stop_id"] == stop_id
                        if m.any():
                            stops_df.loc[m, "lat"] = float(move["lat"])
                            stops_df.loc[m, "lon"] = float(move["lon"])
                    if (
                        "direction" in stops_df.columns
                        and "sequence" in stops_df.columns
                    ):
                        stops_df = _compute_distance_from_prev(
                            stops_df.sort_values(["direction", "sequence"]).reset_index(
                                drop=True
                            )
                        )
                    st.session_state[_sk("updated_stops")] = stops_df
                if pending_depot_now and "depot_id" in depots_df.columns:
                    for depot_id, move in pending_depot_now.items():
                        m = depots_df["depot_id"] == depot_id
                        if m.any():
                            depots_df.loc[m, "lat"] = float(move["lat"])
                            depots_df.loc[m, "lon"] = float(move["lon"])
                    st.session_state[_sk("updated_depots")] = depots_df
                st.session_state[pending_stop_key] = {}
                st.session_state[pending_depot_key] = {}
                st.toast("地図のドラッグ移動を適用しました", icon="✅")
                st.rerun()
        with pc2:
            if st.button("🗑️ 地図の移動変更を破棄", key=_sk("discard_map_drag_moves")):
                st.session_state[pending_stop_key] = {}
                st.session_state[pending_depot_key] = {}
    ...
    with st.form(key=_sk("map_add_form"), clear_on_submit=True):
        ...
        with fc[1]:
            f_lat = st.number_input(
                "緯度",
                value=clat if clat else 0.0,
                format="%.6f",
            )
        with fc[2]:
            f_lng = st.number_input(
                "経度",
                value=clng if clng else 0.0,
                format="%.6f",
            )
        ...
        if submitted and f_name and f_lat != 0.0 and f_lng != 0.0:
            if "バス停" in add_type:
                ...
                st.session_state[_sk("updated_stops")] = stops_df
                st.success(
                    f"バス停 '{f_name}' を追加しました ({f_lat:.4f}, {f_lng:.4f})"
                )
            else:
                ...
                st.success(
                    f"車庫 '{f_name}' ({loc_str}) を追加しました ({f_lat:.4f}, {f_lng:.4f})"
                )
    ...
    with map_tabs[0]:
        ...
        for c in stops_df.columns:
            if c not in edited_stops_map.columns:
                edited_stops_map[c] = (
                    stops_df[c].values[: len(edited_stops_map)]
                    if len(stops_df) >= len(edited_stops_map)
                    else ""
                )
    ...
    with sc2:
        if _sk("last_save_map") in st.session_state:
            st.success(
                f"保存しました: {', '.join(st.session_state[_sk('last_save_map')])}"
            )
