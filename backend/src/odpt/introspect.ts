...
export type FieldStatWithRate = FieldStat & { presentRate: number };

export interface IntrospectResult {
  sampleCount: number;
  fields: FieldStatWithRate[];
}
...
export function introspectRecords(
  records: unknown[],
  sampleLimit = 200
): IntrospectResult {
  ...
  const fields: FieldStatWithRate[] = [...stats.values()].map((s) => ({
    ...s,
