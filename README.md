...
2026-09-21: [NoRel候補の日別路線検査を修正](docs/notes/ROUTE_BAND_SERVICE_DAY_FIX_20260921.md)。翌日の路線変更を7日全体の路線混在と誤判定していました。同日固定と帰庫・時間制約を維持して修正し、関連134 tests通過。元候補の約133円のStage1目的値差は採用済み費用削減ではなく、gap3.78%で目標未達です。BESS追加予備・復元なしを保ち、別固定版で5月だけ再診断します。
