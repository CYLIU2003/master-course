...
[![Tests](https://img.shields.io/badge/tests-294%20passed-brightgreen.svg)](#13-テスト状況--test-status)
...
- 運用者向けデータガバナンス・ブランチ戦略は [`docs/notes/DATA_GOVERNANCE_AND_BRANCH_STRATEGY.md`](docs/notes/DATA_GOVERNANCE_AND_BRANCH_STRATEGY.md) を参照。
...
```text
master-course/
├── data-prep/  producer app
│   ├── refresh / catalog / explorer
│   └── writes only to data/seed + data/built
├── data/
│   ├── seed/   committed Tokyu seed masters
│   └── built/  regenerable Parquet datasets
└── main app (frontend/ + bff/ + src/)
    ├── frontend/  Tokyu research UI
    ├── bff/       sole frontend-facing API
    └── src/       research core + file loaders
```
...
- **main は Tokyu Bus 専用研究アプリ**。runtime で multi-operator 一般化をしない
...
main app の標準構成では `bff.main:app` のみを frontend proxy 先にします。data-prep API を使う場合は別ポートで独立起動してください。
...
**任意: ターミナル 3 — data-prep API:**

```bash
uvicorn main:app --app-dir data-prep/api --reload --host 0.0.0.0 --port 8100
```
...
> 起動後、アプリは seed master を読み込み、`data/built/<dataset_id>/` があればその manifest / Parquet を利用します。built dataset が無い場合も app は落ちず、Tokyu seed master の確認と scenario 作成は継続できます。
>
...
├── data-prep/                      # producer app (catalog / explorer / dataset build)
├── data/
│   ├── seed/
│   └── built/
├── docs/
...
│
├── frontend/                       # Tokyu-only main research UI
│   ├── src/
...
| [`docs/notes/DEVELOPMENT_NOTES.md`](docs/notes/DEVELOPMENT_NOTES.md) | 研究実験ログ / 実験結果 / バグ修正履歴 |
| [`docs/notes/DATA_GOVERNANCE_AND_BRANCH_STRATEGY.md`](docs/notes/DATA_GOVERNANCE_AND_BRANCH_STRATEGY.md) | 運用者向けデータガバナンス / ブランチ戦略 |
