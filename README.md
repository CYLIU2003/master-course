...
**最新の月別12週計算は、修正済みの固定版 `829e3983` で2026-09-14 19:15 JSTに開始しました。開始時点は0/12週、1月の新規Prepareを実行中です。** 全Stage 2でAggregate=0/Presolve=0、SOC・数値許容誤差を保持。47入力ファイルの同一hash、管理JSONの参照先だけの移設、凍結後20テストと元失敗時刻の全経路通過、前後cleanを確認しました。[起動記録](output/monthly_fair_weeks_20260914/presolve_rerun_launch.json)・[新版の独立監査](output/monthly_fair_weeks_20260914/monthly_presolve_independent_audit.json)・[原因と検証](docs/notes/SHIBU21_23_MARCH_SOC_REPLAY_DIAGNOSIS_20260914.md)。旧版の完走週を流用せず、同じ12週を最初から計算します。月別・季節別の最終整理と研究採用は未完了です。 結果の費用順位には時間制限内の探索到達度も関わり、Stage 1 gapを週間費用の誤差幅や季節差の有意性とは扱いません。
