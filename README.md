...
2026-09-23: `LAPTOP-BOLC6VIT` と `LAPTOP-8JS4DQCD` を18台構成の固定controller `c54f684e` に追加しました。2台ともSSH・同一ソース/依存環境/データセット照合と、共有キュー経由のソルバーなし診断ジョブ回収を通過しています。渋24実便の18件バッチは新しいPrepared IDで準備・検査済みですが、実計算は未開始です。新版のシナリオ分類画面もブラウザで確認しました。[配置・実行状態](docs/DISTRIBUTED_COMPUTE.md)。
