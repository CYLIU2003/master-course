...
2026-09-11 計算再開: 全接続候補を保持する日跨ぎ接続の共有ノード表現を追加しました。4路線の実入力では候補271,864,980本を保持したまま接続変数を15,503,580個へ縮約できましたが、なお大規模なため、ユーザーが許可した渋21〜23の3路線でfresh Prepareから順次再実行します。60台・7日間・SOC・BESS・全候補の条件を維持します。小規模BEV/ICE差分4ケースで費用一致と物理検証通過を確認済みで、実週間計算の完了はまだ未確認です。[数学的条件・規模計測・実行設計](docs/notes/EXACT_DEPOT_CONNECTION_FACTORS_20260911.md)。
