...
現行の凍結版12週は搭載32/32/32/64GBを実測した4台だけを登録し、ライセンス失敗した64GB機は割当停止中です。旧8890 controllerは停止し、再起動時も全枠を外部予約として保持。管理画面8868も外部予約2、計算8891だけが2枠を使用します。新しいコードの32GB検査・authority検査は次の正式版起動で使用します。実行中の固定計算コードは変更しません。[適用・試験記録](docs/notes/GUROBI_ADMISSION_POLICY_20260925.md)を参照してください。
