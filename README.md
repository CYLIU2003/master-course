...
前日計画からRollingへの車両契約引継ぎを修正し、月別投入前にも実際のPrepared入力で検査します。通常進捗表は入れ子の失敗原因も表示します。旧試行を保持した新しい固定版への切替・再Prepareは[復旧記録](docs/notes/MONTHLY_ROLLING_RECOVERY_20260925.md)を参照してください。32GB以上・親機の共有2枠・通信不明時の予約保持は継続します。
...
## GurobiのPC・同時実行条件（2026-09-25）

Gurobiを使う最適化・ライセンス試験は、**搭載RAM 32GB以上**のPCへ限定します。OS認識容量31.xGBの32GB機を誤って除外しないよう、Windowsの搭載容量を別に取得します。空きRAM・OS用の余裕・ジョブの必要RAMは従来どおり別途検査します。Gurobi非依存の計算・検算はこの32GB制限の対象外です。

親機の永続キューが共有枠を管理し、実行中・通信不明・終了後の330秒の解放待ち・外部予約を数えます。枠が空くまで新規求解を開始しません。同じWindowsユーザーの `%USERPROFILE%/.master-course/gurobi-authority.json` に指定したキューだけがGurobi枠の管理主体になります。別ポート・別実験のcontrollerで独立した2枠を作らないよう、起動・取得時に拒否します。新しい実験もこの親機キューを使い、実験ごとの入力・attempt・成果物は分離してください。状態不明の枠を消すためにauthorityファイルや予約DBを削除してはいけません。

旧cf4beb97の12週では搭載32/32/32/64GBを実測した4台だけを登録し、ライセンス失敗した64GB機は割当停止中です。旧8890 controllerは停止し、再起動時も全枠を外部予約として保持。管理画面8868も外部予約2、計算8891だけが2枠を使用します。32GB検査・authority検査は新しい稼働版22ef0e0dで使用しています。実行中の固定計算コードは変更しません。[適用・試験記録](docs/notes/GUROBI_ADMISSION_POLICY_20260925.md)を参照してください。

別件として旧固定版の2月はRolling車両台帳の欠落で停止しました。次版では研究採用フラグに関係なく、Rollingに必要な入力台帳を求解前に厳格検査して引き継ぎます。不足宣言を補完したことや12週が完走したことを意味しません。旧結果の上書き・成功への読み替えは行いません。

## 計算の中身を確認する（2026-09-25）

「期間別計画」と「分散計算」の **計算の詳細進捗** で、週ごとの入力準備、配車Stage 1、充電Stage 2、毎時計算、回収・検算を確認できます。「工程・求解・待機理由を見る」を開くと、担当PC、投入時刻、ログ更新時刻、求解器の経過秒・反復・その求解のgap、直近ログ、RAM・ライセンス待ち、失敗理由、試行ID・固定コード版を表示します。

毎時計算の割合は保存済み可行窓数／投入時の予定窓数です。最終翌朝を含むケースは168窓より多くなります。週全体の検算・費用確定は別判定です。配車探索の終了時刻や週全体の最適性を推測したパーセントは表示しません。更新が120秒以上途切れた場合や接続不明の場合は、最後の記録と表示します。

詳細読取は計算コードから独立した、AI不要のスクリプトです。対象を固定した既存 `operation.local.json` と画面配信先を指定して実行します（PowerShell、依存導入済みPythonを使用）。計算開始・再投入・Gurobiライセンス試験は行いません。

```powershell
python tools/research/publish_execution_detail.py --operation <operation.local.jsonの絶対パス> --output <画面配信ディレクトリ>/execution-detail.json --watch
```

`--operation` は複数指定可能です。同じ出力先の多重監視はロックで拒否します。現在の実験は `output/monthly_latest_20260925/operator/07_DETAIL_WATCH.cmd` でも起動でき、既に読取スクリプトが稼働しています。30秒ごとに読取、画面は10秒ごとに更新します。今回の配置・実測・未解決事項は[詳細進捗の記録](docs/notes/EXECUTION_DETAIL_PROGRESS_20260925.md)を参照してください。
...
2026-09-25: 分散計算画面は選択中シナリオのジョブを初期表示し、全件へ切り替えられます。バッチ・月別キャンペーンの全体集計は「全シナリオ・診断」で表示します。別のローカルcontroller（例: 8890）は「監視先ポート」から閲覧できます。別ポートは閲覧専用で、操作はそのcontrollerの画面で行います。監視先は同じブラウザータブで保持し、更新日時と通信停止警告を表示します。

## 週次運用・費用・季節比較を先に出す（2026-09-24）

当面の成果は、既存の渋21〜23について連続7日間の運行・充電、週次費用、
代表週の季節差を示すことです。gap未達だけを理由に検算済み結果の集計を止めません。
元の研究採用判定は保存し、物理・会計を確認できた解を
`VERIFIED_CONDITIONAL_WEEKLY_EVALUATION`として別に表示します。
統合総費用の最適性、実営業所の設備適合、年間連続運用を認定する表示ではありません。

固定実行 `7cb468942d9ec2526d8afc92dc3fa8b7c51b56eb` の12週は原本SHA・日別台帳・
1,704便/週・672区間/週を照合して再集計しました。親機の
`output/weekly_seasonal_20260924/results/weekly_seasonal_report.md`、週次/日別CSV、
全車両運用・SOC表、7日間需給図、季節比較図を参照してください。
これは旧BEV終端条件の成果であり、新しい翌朝SOC条件と混ぜません。

スクリプトの入口は次のとおりです。実行例と配置手順は
[週次成果の運用手順](docs/notes/WEEKLY_SEASONAL_EXECUTION_20260924.md)に記載しています。

- `tools/research/weekly_results.py`: 検証済みの既存月別原本を再照合し、再求解せず表・図へ変換。
- `tools/research/weekly_campaign.py`: 固定DBから三路線の週次Prepare、独立週を指定PCへ投入、回収・検算・図表化。
- `tools/research/weekly_terminal_observer.py`: ローカルスクリプトで監視し、終了/失敗時に一度だけメール送信処理へ引き継ぐ。通常監視にAIを使わない。

代表週は既存の5/12・11/10・2/3・8/4（2025年）を使用。初期状態・物理制約は保持し、
車両は翌朝の運用上限SOC、最終翌朝の充電・受電・費用を含めます。
BESSはPV→バス優先、余剰PVから充電、範囲外へ出さず空なら待機・満杯なら抑制する方針です。
ODPT更新は手動のみで、計算投入時に再取得しません。

## 一つのシナリオで計算期間を選ぶ

シナリオ選択画面では新規作成、既存シナリオの複製、名前・説明の編集ができます。
開いたシナリオの「管理」では起動時シナリオの指定や、確認を伴う削除も行えます。
新規作成は既定のデータセットを使い、別データセットIDを指定することもできます。
存在しないデータセットを指定した場合は作成を拒否し、空のシナリオを残しません。
複製は新しいIDの独立した編集用コピーです。結果を正式比較に流用する根拠にはなりません。

「運行・計算設定」→「対象範囲」では、保存済み路線を系統ごとに確認し、系統全体または
個別の方向・経由地パターンを選べます。検索で非表示になったパターンも、系統全体の
チェックでは一緒に選択されます。「この系統だけを対象にする」で既存の全路線選択から
一系統へ絞れます。出典不明の選択IDがあればこの操作は停止します。画面の系統名は閲覧用で、保存する計算対象は正確な
路線パターンIDです。対象を変えると旧Prepared入力は使えず、「実行」で新しく入力を
準備してください。選択だけでは時刻表原本や車両・距離は追加されません。必要な路線が
一覧にないときは、ODPTの手動取得と固定データ取込みを先に完了してください。

画面の「シナリオを選ぶ」は再利用するシナリオを最初に表示します。従来の月別・日付別の保存ファイルは「旧月別・日付別ファイル」へ切り替えると参照でき、削除されません。分類は保存名による表示上の整理で、計算対象路線を認定するものではありません。

同じシナリオの「運行・計算設定」→「運行条件」で開始日と対象日数を設定します。1日、1週、2〜4週のボタン、または1〜366日の直接入力を使い、保存後に「実行」で入力を準備します。保存時に開始日から連続する日付列を生成し、明示日付列と矛盾すれば拒否します。Prepared入力は期間ごとに別IDで固定されます。**期間を変えて保存すると、そのシナリオの以前のPrepared IDは新規投入に使えません。** 既に投入済みの試行は独立の凍結入力・成果物を参照し、同じシナリオで異なる期間を同時に新規投入する機能はまだありません。

入力準備は日付ごとの時刻表・気象・車両条件が揃っているか別途検査します。1〜366日の入力欄は、その期間のデータや長期求解の成功を保証しません。複数日の正式研究実行は従来どおりブロックされ、診断結果を旧月別12週や研究採用済み結果に読み替えません。

## 全シナリオ共通の分散ジョブ進捗

「分散計算」画面の「キューと実行履歴」は、シナリオ名や路線に依存せず、各ジョブの状態・配布先・処理工程の到達率・直近メッセージ・エラー・試行履歴を表示します。子機が保存した進捗は試行IDとmanifest SHAに結び付けて親機SQLiteへ記録するため、画面や親機を再起動しても残ります。SSH切断中は最後に確認できた値と表示し、通信断だけで完了や失敗と決めません。

`tools/cluster/batch.py` から投入したバッチは、宣言済みタスク数を分母に、親機が成果物を回収・検証したタスク数を進捗率として表示します。失敗・状態不明・未投入も分母から除きません。同じタスクの再試行は最新の試行1件で数えます。子機の工程率は最適化処理のチェックポイントであり、求解器内部の探索率・残り時間・最適性gap・研究採用率ではありません。子機から工程報告のない診断ジョブは「報告なし」と表示します。

下記の渋24月別12週表示は、そのキャンペーンに固有のPrepare・監査記録を補足する表示です。共通のジョブ進捗には専用publisherや静的JSONは不要です。
...
翌朝SOC契約を持つPrepared入力は、対話実行の代表日向けSOC初期値復元ルールで
上書きせず、実行時に契約を再検証します。
...
旧三・四路線診断の `run_exact_seasonal_campaign.py` も、開始時にODPT由来の候補を
再生成しなくなりました。使う場合は先に
`prepare_shibu21_24_seasonal_inputs.py --build-source --source-output <新規候補先>` を
...
2026-09-23: 分散ジョブが通信断で `LOST` になった場合、親機は同じ試行を自動再投入しません。子機に開始記録がない場合だけ、遅延投入を防ぐ記録を置いて未開始を確定し、別IDでの再試行を許可します。開始済みか不確かな試行は予約を保持して照合します。[復旧の扱い](docs/DISTRIBUTED_COMPUTE.md)。
...
2026-09-25: Solcastの2023年10月（2,976件）を追加取得しました。月次マニフェストから再検証した学習対象2022〜2024年は34/36か月・99,360件、残りは2023年11月・12月です。地点・PT15M・7項目の有限値・月内連続性・request/raw SHAを照合済みです。残量不明のため本日は1リクエストで終了し、全36か月が揃うまで3年分の新学習モデルは生成しません。取得状況: [状態JSON](output/seven_day_extension_20260910/training_history_acquisition_status.json)。既存2024年学習モデルと凍結済み計算入力は保持しています。
...
runs from execution SHA `ba5ac4a` preserved the frozen one-thread,
  ...
  and winner-hash gates.  SUNNY selected `76fb6a9b...a516c`; its day-ahead
  ...
  cost is 698,598.628643 JPY. Read-only re-finalization at `3401ad8` records
...
The selector is an explicit `run-optimization` request field and is recorded
in canonical solver metadata; it is deliberately not a loose Prepare setting.
An initial attempted 264-trip pair was discarded when both requests resolved
...
SSH監視は一度の一時的タイムアウトを再試行し、続けて失敗した端末だけを割当対象から外します。機器管理画面は長いSSHコマンドの例外文ではなく接続確認の案内を表示します。実機ごとの現行の役割とライセンス確認結果は [分散計算の手順](docs/DISTRIBUTED_COMPUTE.md) に記録しています。

機器管理では、親機からPCごとにGurobi計算・Gurobi不要のALNS・両方・診断のみを指定できます。設定は保存され、自動割当と配布先指定の双方で守られます。
...
初回の渋21 1日試行は、Stage 1が同日内の帰庫を間に挟めない勤務を作り、独立タイムライン検証で停止しました。明示カットの再試行でも同種の問題が残ったため、勤務断片の接続判定を物理タイムラインと同じ「帰庫が可能」に修正し、全段階で修正済みの `lazy` 分離を使います。旧失敗試行は診断履歴のまま保持し、新しい結果に数えません。

7日間の投入前には、ローカルの空きRAMからシステム予約分を除き、18 GB以上かを検査します。これは渋21専用の投入基準であり、メモリ不足にならない保証ではありません。基準未満なら求解を起動せず原因を保存します。
...
## 2026-09-25: シナリオ内の期間別計画

「仮・正式用」の1～12月代表週は、ひとつのシナリオ内の「期間別計画」で管理します。共通条件と期間・attemptを分け、週ごとの状態、準備・検算率、失敗理由を確認できます。[操作・AIなしCLI・検証範囲](docs/notes/SCENARIO_PERIOD_PLANS_20260925.md)。


2026-09-25の管理画面は http://127.0.0.1:8868/ で「仮・正式用」→「期間別計画」を開きます。12代表週を登録済みです。管理実行版は00dcc478、稼働中の週次求解は0964b783で別管理です。今回のSOC時刻・証拠保存修正は新版コードに含まれますが、新版の実規模7日間はまだ未確認です。


2026-09-25: 他チャットの路線ブラウザーと現行の期間管理を統合しました。[統合内容・修正・検証範囲](docs/notes/CROSS_CHAT_INTEGRATION_20260925.md)。営業所別閲覧に加え、対象設定には従来の全路線検索も残しています。

### 新版の月別12代表週（2026-09-25）
...
### PCごとのメモリ余裕を残す（2026-09-25）

計算の要求メモリとは別に、子機4GiB以上、親機6GiB以上を追加の余裕として確保します。18GiB要求なら子機は空き物理RAMとWindows commit余地の両方が22GiB以上必要です。既存の使用量を引いた空き容量からさらに差し引く値で、他attemptの予約も控除します。容量不足・状態不明なら待機し、ソルバー条件を黙って下げません。これは投入条件であり、実行中の他アプリの増加やソルバーの予算超過まで防ぐOS上のハード上限ではありません。
...
### 現行試行と旧版エラーの見分け方（2026-09-26）

詳細進捗は監視先コントローラーの固定SHAと照合し、現行版の記録と、別版でも実行中/通信不明の記録を先に表示する。旧版の終了記録は折りたたみ履歴へ残す。各カードに計算版と試行IDを表示する。件数は現在表示している試行が対象で、12週全体の完了率と取り違えない。別版だけの週数を別記する。nativeログの作成だけでは求解開始とせず、Optimize/Presolve/求解行が確認できるまでモデル構築と表示する。


### 2026-09-26: 週次計算の省メモリ求解設定

`tools/research/weekly_campaign.py` は根LPに既存の `bounded_presolve_dual`（双対単体法）を使用します。32 GB機でタスク16 GiB、native14 GiBの上限を維持します。以前のbarrier実行は根LPの初回反復前にメモリ上限へ達しました。旧試行・通知は保存し、新しいclean SHA・新規Prepare・別attemptで再実行します。上限緩和や便・SOC条件変更は行いません。設定変更は完走保証ではなく、実行結果はフロントの現行SHA・試行IDで確認してください。


### 2026-09-26: 配車から充電へのメモリ引継ぎ

Phase 3で配車候補が1件の経路は、確定配車とStage 1の数値を保持してから、不要なnative配車モデルを解放してStage 2を開始します。共有Gurobi Env・予約枠は維持します。複数候補探索は配車モデルを再使用するため対象外です。実行中の固定版へ途中適用せず、次の新規固定版から適用してください。


毎時充電のモデルは、その窓の結果・診断の抽出が終わった時点で解放します。共有Envとライセンス予約は週次ジョブの終了まで保持し、過去窓のモデルを積み上げません。再試行は旧attemptの終了確認、新しいclean版でのPrepare後に行い、機器別メモリ予算は維持します。

毎時の全予測結果は各stepのJSONに保存し、週次集計中のメモリには採用済み区間の電力量とSOC両端だけを保持します。先読み区間は週次費用へ足し込まず、元の結果と日境界引継ぎは変更しません。


### 計算中プロセスのメモリ実測（2026-09-26）

分散計算の詳細進捗では、担当worker上のPIDと生成時刻を照合し、使用中RAM・起動後のRAM最大・専用コミット・投入時の計算予算を表示する。RAMと専用コミットは合算しない。照会不能・PID不一致は未取得とし、0や終了に変換しない。Windows 64bitの読取り専用APIを使い、Gurobi環境は起動しない。各attemptの親機側 `operations/process-memory.jsonl` に30秒間隔の観測を残す。これはプロセス単体の観測であり、OSの強制メモリ上限や週次計算の成功保証ではない。


### 機器別計算の安定性優先（2026-09-26）

速度より完走を優先する。32GB機では計算予算16GiB程度、64GB機では32GiB程度を上限とし、実測空きRAM・OS用余裕・既存予約・コミット余裕が不足すれば待機する。Gurobiは32GB以上の機器だけへ割り当てる。通信不明や終了未確認で別試行を重ねず、同じ試行を照合する。制約・便・SOCを削って成功扱いにしない。分散計算画面の実RAM、最大RAM、予算、読取時刻を確認する。時間上限時の可行解と最適性証明、週全体の物理・会計検算は区別する。


入力準備だけを行うキャンペーンも分散計算の詳細進捗へ表示できる。`prepare-only-state.json` のコード版・シナリオ・プロセス生成時刻を照合し、「準備済み・求解未投入」「親機の空きRAM待ち」「状態不明」を区別する。表示は求解や再投入を行わない。
