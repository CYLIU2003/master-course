...
2026-09-21 初期配車チェックのログ不具合を修正: 旧27253fa8はPrepare後、本Stage1開始前に停止しました（成功0/12）。内部チェックの「保存先なし」とログ有効フラグの矛盾が原因です。内部だけログ無効とし、本求解・rollingの記録を維持。再現テストと関連94 tests通過。研究条件を変えず新しい固定版へ移行します。[原因・修正・今回の実行先](docs/notes/MONTHLY_AUXILIARY_LOGGING_RECOVERY_20260921.md)。以下の27253fa8起動記録は停止済み旧版です。
