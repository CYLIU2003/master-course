...
同じシナリオの「運行・計算設定」→「運行条件」で開始日と対象日数を設定します。1日、1週、2〜4週のボタン、または1〜56日の直接入力を使い、保存後に「実行」で入力を準備します。保存時に開始日から連続する日付列を生成し、明示日付列と矛盾すれば拒否します。Prepared入力は期間ごとに別IDで固定されます。**期間を変えて保存すると、そのシナリオの以前のPrepared IDは新規投入に使えません。** 既に投入済みの試行は独立の凍結入力・成果物を参照し、同じシナリオで異なる期間を同時に新規投入する機能はまだありません。

入力準備は日付ごとの時刻表・気象・車両条件が揃っているか別途検査します。1〜56日の入力欄は、その期間のデータや長期求解の成功を保証しません。複数日の正式研究実行は従来どおりブロックされ、診断結果を旧月別12週や研究採用済み結果に読み替えません。
