...
2026-09-21 月別再実行: ユーザー承認により、BESS補助運用の修正版で同じ12週を新規実行する設定を追加しました。旧月別計算は実プロセスなし・完了済みと確認。関連101 tests通過、別のclean固定版から新規Prepareし、通常処理はスクリプトのみで行います。[今回の条件・状態・配信](docs/notes/MONTHLY_AUXILIARY_EXECUTION_20260921.md)。
