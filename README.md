...
2026-09-21: [配車2条件の結果と充電可能時間の表現改善](docs/notes/STAGE1_CHARGE_WINDOW_SUPPORT_20260921.md)。固定4f979982は両条件の物理検証・充電gap0.368%を通過。配車は費用改善0円・認証gap6.91%で、NoRelなしは600秒、ありはメモリ上限で停止しました。充電可能時間の繰返しを等価な端点表現へ置き換える実装を追加し、同一条件の新規比較を準備しています。5月の費用・速度改善や全12週の新結果は未確定です。
