...
│  サイドバー（左）    │  メインエリア（右）                     │
│  ─────────────────  │  ──────────────────────────────         │
│  ⚙️ 設定方法の選択   │  📋 現在の設定概要 (メトリクス)         │
│  ├─ 手動設定         │  └─ 詳細設定（バス/便/充電器/エネルギー）│
│  └─ JSON インポート  │                                         │
│                     │  🧮 ソルバー実行タブ                     │
│  📐 システム規模     │  ├─ Gurobi (MILP)                       │
│  🚌 車両性能        │  ├─ ALNS                                 │
...
「**🏢 営業所管理**」タブでは、営業所・車両・行路（便チェーン）を管理できます。

1. **営業所の管理**: 営業所の追加・編集（名称、所在地、駐車容量、系統接続容量など）
2. **車両の管理**: 各営業所に車両を登録（車両タイプ、バッテリー容量、電費、担当路線）
3. **行路の編成**: 時刻表から便を選んで便チェーン（行路）を作成。1台の車両が1日に複数便を担当する順序を定義
