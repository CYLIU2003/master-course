...
2026-09-12 最新訂正: 凍結 `a812aeb2` の冬・春は各168時間と独立物理検証を通過しましたが、春の確定会計で契約超過単価の未設定値が0円となり、15,522.689811円の過少計上を検出しました。春の会計成立は撤回し、夏のPrepare中に停止しました。未指定単価をnative solverと同じ500 JPY/kWhへ統一し、実行超過量と料金の照合を追加しました。新規12件・関連97件、独立コードレビューP0/P1ゼロ、全体回帰 **2,240 passed / 既存資料2 failed** を確認済みです。保持計画の再集計では物理量を変えず料金差だけを修正できました。修正後の四季再実行は未完了です。[原因・数式・検証](docs/notes/CONTRACT_OVERAGE_PRICE_DEFAULT_20260912.md)。以下は旧SHAで当時の検査を通過した履歴であり、今回の料金検査を通過した証拠ではありません。

2026-09-12 最新: 凍結 `43fe848f` で冬・春・夏の各168時間、独立物理検証と最終会計を通過しました。秋は159時間通過後、許可済みの有料契約超過を検証側が一律拒否して停止しました。超過量と費用計上量の一致を必須とする修正は、関連41テスト・独立コードレビューP0/P1ゼロ・全体回帰 **2,228 passed / 既存資料関連2 failed** を確認済みです。保持状態から残り9時間の再現も通過しました。新しいclean SHAによる全四季の計算と秋の週間費用は未成立です。[原因・数式・検証](docs/notes/CONTRACT_OVERAGE_VALIDATION_20260912.md)。以下の計算記録は各旧SHAの履歴です。

2026-09-12 最新の計算状況: 凍結 `803f8f9f` の渋21〜23冬週は **168/168時間・独立物理検証・最終会計を通過**し、日別費用との差は0円でした。春週で停止した不要なBESS固定値検査と、未観測PVの充電を信用した放電指令を修正しました。保持状態からの101〜167時の求解・実測PV実行はすべて通過。全体回帰は **2,212 passed / 既存資料関連2 failed**、独立実装レビューのP0/P1は0件です。新しいclean SHAでの全四季検証は未完了です。[BESS境界修正](docs/notes/ROLLING_BESS_REFERENCE_POLICY_20260912.md)、[PV不足の原因・制約・検証](docs/notes/ROLLING_PV_EXECUTION_RESERVE_20260912.md)。

2026-09-12 計算時間の修正: 毎時の同一時刻表に対する事前検査を、全構造入力のハッシュ一致時だけ再利用します。SOC・燃料・PVの求解と独立検証は各時間で継続します。[検査の同等性と測定](docs/notes/STRICT_PRECHECK_REUSE_20260912.md)。

2026-09-12 続報: 凍結 `cdd66532` で冬週の週間計画・物理検証と15時間のrollingが通過しました。16回目は境界SOCの約 `7.1e-15 kWh` の丸め差を範囲外とする厳密浮動小数点比較で停止したため、保存値を変更せず既存の数値許容差へ統一しました。4季節の週間実行・会計はまだ未成立です。[証拠と修正](docs/notes/ROLLING_TERMINAL_CHARGE_SESSION_20260912.md)。

2026-09-12 計算再開: 凍結 `2bd7cc9f` の渋21〜23冬週は全78,647,760接続候補を保持し、週間day-aheadの物理検証を通過しました。最初のrolling窓が継続中の充電に終了処理を追加して停止したため、前日予測計画の境界状態を引き継ぐ修正を入れました。修正後の同一入力・最初の窓は実行可能です。4季節の168時間rolling・最終会計、新しいclean SHAでの再実行は未完了です。[原因・数式・検証](docs/notes/ROLLING_TERMINAL_CHARGE_SESSION_20260912.md)。

2026-09-11 計算再開: 渋21〜23の冬週に有限ICE燃料制約を追加した再実行は、Stage 1の120秒内に解を得られず停止しました。探索初期候補の週末SOC・充電条件を修正し、更新候補は共有充電計算と独立物理検証に通過しました。全78,647,760接続候補・60台・燃料144 L/下限16 Lを保持した全MILPの再実行と、4季節の週間rolling・最終会計はまだ未完了です。[原因と修正](docs/notes/PHASE3_FINITE_ICE_FUEL_20260911.md)、[全候補表現と規模計測](docs/notes/EXACT_DEPOT_CONNECTION_FACTORS_20260911.md)。

2026-09-11 最終状況: Electronの主要操作移行とportable版の検証は完了しました。渋21〜24の4季節は完全Prepareをすべて通過しましたが、冬週のモデル構築でPCのメモリー残量が逼迫したため停止し、他3週は未実行です。週間費用・研究採用は未成立です。統計用の巨大接続リストの重複生成を除去しましたが、実solverの規模問題は残ります。[入力4週の状態・停止証拠・残課題](docs/notes/SHIBU21_24_RESOURCE_BLOCK_20260911.md)。
