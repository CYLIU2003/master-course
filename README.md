...
| 見たい内容 | 参照先 |
|---|---|
| 研究目的と概要 | [1. このシステムが解く問題（先生向け概要）](#1-このシステムが解く問題先生向け概要) |
| 制約・目的関数の実装状況 | [3. 実装状況と研究フェーズ](#3-実装状況と研究フェーズ) |
| 起動手順 | [4. セットアップと実行手順](#4-セットアップと実行手順) |
| 主要画面と操作フロー | [5. 東急全体最適化の推奨フロー](#5-東急全体最適化の推奨フロー) |
| 最適化計算の仕組み（数理モデル） | [2. 最適化モデルの説明](#2-最適化モデルの説明) |
| アーキテクチャ・ファイルレイアウト | [6. システム構成](#6-システム構成) |
| 開発ノート・既知問題 | [7. 既知の注意事項とトラブルシューティング](#7-既知の注意事項とトラブルシューティング) |
| 監査結果と再現手順 | [10. 実測監査](#10-実測監査) |
| AI エージェント向けアーキテクチャ仕様 | [11. AI エージェント向けアーキテクチャ仕様](#11-ai-エージェント向けアーキテクチャ仕様) |
...
- ICE バスの燃料費 ＋ 電気代（時間帯別 TOU）＋ デマンド料金 ＋ 車両固定費
