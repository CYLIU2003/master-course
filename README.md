...
4月の停止原因について、元の入力・状態を使った1時間の再検査で、Stage 2の数値集約を無効化すると同じ制約・許容誤差で充電計画が成立しました。[原因・修正・再実行方針](docs/notes/SHIBU21_23_APRIL_NUMERIC_DIAGNOSIS_20260914.md)。全月共通の設定として修正を検証中で、新しい固定版による1〜12月の再実行を準備しています。初回試行の完了3週を新版の結果とは扱いません。
...
2026-09-14 途中経過: **1〜3月が完走。4月はhour 023の充電求解が失敗し、キャンペーンは停止しています。** 完了3週は各168時間・672 slot、物理違反0件、確定会計と日別台帳の照合を通過しました。確定費用は1月4,309,715.542368円、2月4,221,549.725308円、3月4,430,823.263078円です。[月別結果表と原本への参照](docs/notes/SHIBU21_23_MONTHLY_RESULTS_20260914.md)。4月は23/168時間まで受理、週間会計は未成立。5〜12月は未実行。保存制約を使って停止原因を調査しています。
