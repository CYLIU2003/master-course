...
- `scenario_fleet_contract_v2` makes the exact active vehicle set from the
  ...
  `v5_pv_rated_output_authoritative` retains the explicit fleet-state and
  same-service-date counterfactual contracts introduced in v4, materializes the
...
- Stage 2 remains the exact fixed-assignment binary charger/SOC/PV/BESS check.
  ...
  Before each exact-count or `used BEV >= K` solve, the search may provide a
  partial MIP start that moves all duties from one or more active vehicles to
  distinct unused vehicles of the opposite powertrain. It matches the complete
...
- 2026-08-02: `PV定格出力 [kW]（最適化入力）` を営業所別PV規模の主入力として復元した。`pv_capacity_kw_manual_override=true` の行は `pv_capacity_kw` をそのまま使用し、`推定PV設置可能面積 = pv_capacity_kw / panel_power_density_kw_m2`、`定格出力換算の推定営業所面積 = 推定PV設置可能面積 / usable_area_ratio` を逆算する。実測の `depot_area_m2` は上書きしない。Solcast は容量係数/時系列形状として扱い、Prepare/canonical builder で `pv_generation_kwh_by_slot = pv_capacity_kw * capacity_factor * Δt[h]` を再構築する。営業所面積から定格出力を再推定する補助操作と、旧面積入力行の互換モードは残す。Prepare schema は `v5_pv_rated_output_authoritative` へ更新したため、変更前の prepared input / run 出力は再利用しない
- 2026-04-10: 営業所PV容量を固定入力から営業所面積由来へ変更した。`営業所別充電器管理` で `営業所面積 [m²]` を保存し、`depot_area_m2 * 0.35 * 0.20` で `pv_capacity_kw` を導出する。Solcast は容量係数/時系列形状として扱い、Prepare/canonical builder で `pv_generation_kwh_by_slot = derived_capacity_kw * capacity_factor * Δt[h]` を再構築する。`depot_area_m2` が null または 0 以下なら PV は無効で、旧 `pv_capacity_kw` だけではPVを有効化しない。この変更はPV規模の数学的意味を変えるため、旧固定容量PVのKPIとは直接比較しない（2026-08-02以降は明示的な定格出力入力を再び正式サポート）
...
- 2026-04-30: `solcast_pv_proxy_v1` は運行日当日の実PV形状を読むため検証用/Oracle寄りとして明示し、通常の予報シミュレーション用に `solcast_typical_pv_proxy_v1` を追加した。`scripts/weather/build_solcast_typical_curves.py` は過去 Solcast profile を日積算 capacity factor hours で rainy/cloudy/sunny に分類し、各時刻の平均24h capacity factor曲線を作る。`scripts/weather/build_solcast_typical_proxy_forecast.py` はその代表曲線から forecast JSON を作り、BFF は weather policy 有効時に代表曲線を canonical `depot_energy_assets` のPV列へ時刻対応で反映する。Solcastは形状だけに使い、PV規模は明示的な定格出力、または互換用の `depot_area_m2 * 0.35 * 0.20` で決める。EV/ICE の選択は weather bias ではなく、PV・買電・燃料費・需要料金・SOC制約・車両制約から最適化が判断する。充電または給油だけを行った未運用車両も `graph/vehicle_timeline.csv` と `vehicle_operation_diagrams/all_vehicles.svg` に出力する
...
- 2026-03-31: 営業所別エネルギー資産は `depot_energy_assets` で `pv_capacity_kw` と `bess_energy_kwh / bess_power_kw` を編集できる前提に整理し、日別 PV capacity factor から複数日 horizon 用の発電列を再構築できるようにした。2026-04-10から面積由来を既定としたが、2026-08-02以降は `pv_capacity_kw_manual_override=true` の定格出力を主入力として正式サポートし、面積由来は明示的な補助・互換モードとする
...
- `営業所別充電器管理` の `PV定格出力 [kW]（最適化入力）` が営業所別PVの規模入力で、`simulationSettings.depotEnergyAssets` / `simulation_config.depot_energy_assets` には `pv_capacity_kw`、`pv_capacity_kw_manual_override=true`、Solcast由来の容量係数・発電列が保存される
- 定格出力入力時は `推定PV設置可能面積 = pv_capacity_kw / 0.20`、`定格出力換算の推定営業所面積 = 推定PV設置可能面積 / 0.35` として監査値を保存する。`面積推定PV設備容量` は逆算した設置面積へ同じ出力密度を掛け戻した整合確認値で、入力した定格出力と一致する。実測の `depot_area_m2` は変更しない
- `pv_capacity_kw_manual_override` がない旧行だけは、互換モードとして `pv_capacity_kw = depot_area_m2 * 0.35 * 0.20` を用いる。UIの「営業所面積から定格出力を再推定」を押すと、この面積由来値を新しい定格出力入力へ反映できる
...
- `solcast_typical_pv_proxy_v1` は代表24h capacity factor曲線を時刻対応で切り出し、選択したPV定格出力で再スケールする。天気クラスは SOC / 初期SOC / EV-ICE bias を変更せず、PV 発電見込みの差だけを最適化へ渡す
