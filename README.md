...
2026-09-11 計算再開: 渋21〜23の冬週は、全78,647,760接続候補を保持して運行割当と充電計画まで到達しました。ただしICE燃料の下限違反505件で物理検証が不成立となり、他3季節を停止しました。車両別の有限燃料制約と固定割当の燃料検査を追加し、初期燃料144 L・下限16 Lを維持して再実行します。週間計算・最終会計はまだ未完了です。[原因と修正](docs/notes/PHASE3_FINITE_ICE_FUEL_20260911.md)、[全候補表現と規模計測](docs/notes/EXACT_DEPOT_CONNECTION_FACTORS_20260911.md)。
