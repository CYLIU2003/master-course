...
初回の渋21 1日試行は、Stage 1が同日内の帰庫を間に挟めない勤務を作り、独立タイムライン検証で停止しました。新しい固定版では `stage1_fragment_transition_cut_mode=explicit_root` を1日・7日・12週共通にして、この接続条件をStage 1のモデルに明示します。旧失敗試行は診断履歴のまま保持し、新しい結果に数えません。
