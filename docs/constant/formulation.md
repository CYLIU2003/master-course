...
> [!IMPORTANT]
> **本文書は「研究として最終的に目指す完全版定式化」の仕様書です。core の現時点実装とは差分があります。**
>
> 本文書を単独で読むと、実装より広い機能が「実装済み」と誤解されるおそれがあります。
> **「現在 core で実際に動くもの」の説明として正確なのは README.md（第1〜3章）です。**
>
> | 確認したいこと | 参照先 |
> |--------------|--------|
> | 現在 core で動く最適化モデルの概要 | [README.md 先生向け要約](../README.md#このシステムが何をしているか先生向け要約) |
> | 制約・目的関数の実装状況（✅/🔶/❌） | [implementation_status.md](implementation_status.md) |
> | 研究として目指す完全版（本文書） | 本文書（以下） |
>
> **主な実装差分（2026-03-18 時点）：**
>
> - **CO₂費・電池劣化費**：目的関数への組み込みは実装済み。ただしデフォルト値は 0（無効）。パラメータ設定で有効化する。
> - **C1（便割当）**：理論式は等式制約 $\sum_k y_j^k = 1$ だが、実装は罰則付き緩和（欠便変数 $u_j$ 導入）。
> - **C13/C14（充電器制約）**：充電器への個別割当ではなく、合計台数・合計 kW 容量の上限制約として実装。
> - **C20/C21（ピーク判定）**：tariff テーブル未設定時は価格中央値による近似。
> - **C6〜C8（SOC 遷移）**：Big-M 形式ではなく連続時間近似。

---

## ステータス
- 分類: 研究仕様書（完全版定式化）
- 用途: 混成フリート + PV を含む MILP の目標定式化を確認する
- 注意: **本文書の記述が全て実装済みであるとは限りません。実装状況は上記の差分表を参照してください。**

### 位置づけの3層整理（2026-03-18）

1. 研究として最終的に目指す定式化（本書）
2. 2026-03-18 時点で core に実装済みの範囲（`implementation_status.md`）
3. 未実装だが修論計画上で今後導入する範囲（同上のフェーズ計画）
...
$$
t_i^{arr} + t_i^{turn} + t^{buf} + t_{ij}^{dh} \leq t_j^{dep}
$$

ただし $t_i^{arr}$：便 $i$ の到着時刻，$t_i^{turn}$：便 $i$ 終点での最小折返し時間（turnaround），$t^{buf}$：接続準備・運行余裕として別途加える非負のバッファ，$t_{ij}^{dh}$：$i$ 終点から $j$ 始点への回送時間，$t_j^{dep}$：便 $j$ の出発時刻。実装では `turnaround_buffer_min` を停留所別または既定の最小折返し時間に加算し、その後に回送時間を加える。
...
> 追補（2026-03-24, core_pv B案）
> 現行 core では営業所ごとに PV→BESS→EV 充電を明示する拡張を導入し、
> C15-C18 は以下の営業所別フロー制約（C15-C27）へ段階移行する。
> 旧式（`g_t + pv_t^{ch}`）は互換説明として残す。
...
#### (C22) PV→BESS 収支（営業所 $d$）

$$
PV_{d,t}^{gen} = PV2B_{d,t} + Curt_{d,t}^{PV}
$$

#### (C23) EV充電需要バランス（営業所 $d$）

$$
B2Bus_{d,t} + G2Bus_{d,t} = Charge_{d,t}
$$

#### (C24) BESS SOC 遷移（営業所 $d$）

$$
SOC^B_{d,t+1} = SOC^B_{d,t} + \eta_d^{ch}(PV2B_{d,t}+G2B_{d,t}) - \frac{1}{\eta_d^{dis}}B2Bus_{d,t}
$$

#### (C25) SOC 境界

$$
SOC_{d}^{B,min} \le SOC^B_{d,t} \le SOC_{d}^{B,max}
$$

#### (C26) BESS 充放電出力上限

$$
PV2B_{d,t}+G2B_{d,t} \le P_{d}^{B,ch,max}\Delta t,
\quad
B2Bus_{d,t} \le P_{d}^{B,dis,max}\Delta t
$$

#### (C27) Grid→BESS 許可フラグ

$$
allow\_grid\_to\_bess_d = 0 \Rightarrow G2B_{d,t}=0
$$
...
> **重要：O1〜O4 はすべて 1 つの式 $C_{total}$ に足し合わせて同時に最小化します。**
>
> O1（ICE 燃料費）を単独で最小化すれば「ICE を 1 台も使わない」が自明な解となり、
> O2（電気代）を単独で最小化すれば「BEV を 1 台も充電しない（= ICE のみ運用）」が自明な解となります。
> これらを分離して扱っても研究上の意味はありません。
>
> $C_{total} = O1 + O2 + O3 + O4$ を 1 本の式として最小化することで、
> ソルバーは「ICE を使えば燃料費（O1）が増え、BEV を充電すれば電気代（O2）が増える」
> というトレードオフを内在化し、**最適な混合比率・充電スケジュール** を自動で決定します。

$$
\min \quad C_{total} = \underbrace{C_{fuel} + C_{elec} + C_{dem} + C_{veh}}_{\text{常に有効}} + \underbrace{\sum_{j \in \mathcal{J}} \pi \cdot u_j}_{\text{欠便ペナルティ（常に有効）}} + \underbrace{C_{CO_2}^{*} + C_{degr}^{*}}_{\text{パラメータ設定時のみ有効}}
$$
...
\sum_{j \in \mathcal{J}} f_k(j) \cdot y_j^k
...
| 記号 | 意味 | 型 |
|------|------|----|
| $y_j^k$ | 車両 $k$ が便 $j$ を担当するか（1=担当） | 0/1 整数（決定変数） |
| $x_{ij}^k$ | 車両 $k$ が便 $i$ → $j$ の回送接続をするか | 0/1 整数（決定変数） |
| $c_f$ | 軽油単価（円/L） | パラメータ |
| $f_k(j)$ | 便 $j$ の燃料消費量（L） | パラメータ（trip.fuel_l） |
| $f_k^{dh}(i,j)$ | 便 $i$→$j$ 回送の燃料消費量（L） | パラメータ（距離×燃費） |
| $\mathcal{K}^{ICE}$ | ICE 車両の集合 | 集合（vehicle_type が BEV/PHEV/FCEV 以外） |

**意味：** ICE 車両が担当した便の燃料費と回送時の燃料費の合計。O2 と合算して最小化することで、「ICE を使うほど O1 が増えるが BEV に置き換えるほど O2 が増える」というトレードオフが生まれ、最適な ICE/BEV 混合比率が決まる。

#### (O2) 実充電源ベース電力料金（core_pv B案）

$$
C_{elec} = \sum_{d,t} \left[
p_t^{grid}(G2Bus_{d,t}+G2B_{d,t})
+ c_d^{bess}\,B2Bus_{d,t}
+ c^{curt}\,Curt_{d,t}^{PV}
\right]
$$
...
| 記号 | 意味 | 型 |
|------|------|----|
| $g_t$ | スロット $t$ での系統買電量（kWh） | 連続（決定変数、$\geq 0$） |
| $p_t^{grid}$ | スロット $t$ の系統電力単価 円/kWh（TOU） | パラメータ（slot.grid_buy_yen_per_kwh） |
| $\mathcal{T}$ | 時間スロットの集合 | 集合 |

**意味：** 実際の充電源（Grid/BESS）に応じて課金する。評価器では走行時に仮計上し、充電実績があれば source 単価で遡及上書きする台帳方式を採用する。
...
| 記号 | 意味 | 型 |
|------|------|----|
| $W^{on}$ | オンピーク期間の最大需要電力（kW） | 連続（決定変数、$\geq 0$） |
| $W^{off}$ | オフピーク期間の最大需要電力（kW） | 連続（決定変数、$\geq 0$） |
| $p^{dem,on}$ | オンピーク・デマンド単価（円/kW） | パラメータ |
| $p^{dem,off}$ | オフピーク・デマンド単価（円/kW） | パラメータ |

**意味：** オンピーク・オフピーク別の月間最大需要電力に対して課金される。充電を分散させると最大需要が抑えられ料金が下がる。$W^{on} \geq \bar{p}_t$ （オンピーク全スロット）という制約で最大値を追跡する。

#### (O4) 車両固定費

$$
C_{veh} = \sum_{k \in \mathcal{K}} c_k^{veh} \cdot z_k
$$
...
| 記号 | 意味 | 型 |
|------|------|----|
| $z_k$ | 車両 $k$ が 1 日以上使われるか（1=使用） | 0/1 整数（決定変数） |
...
$$
C_{penalty} = \sum_{j \in \mathcal{J}} \pi \cdot u_j
$$
...
| 記号 | 意味 | 型 |
|------|------|----|
| $u_j$ | 便 $j$ が担当不能（欠便）か（1=欠便） | 0/1 整数（決定変数） |
| $\pi$ | 欠便ペナルティ単価（円/便） | パラメータ（最低 10,000 円/便） |

**意味：** 欠便を実質的に禁止するための大きな罰則。O1〜O4・CO₂費・劣化費の設定に関わらず、コード上では **すべての費目の後に無条件で加算**される（C1 の罰則付き緩和実装の中核）。

### 5.2 CO₂費・劣化費（目的関数化オプション）

#### CO₂費（`co2_price_per_kg > 0` のときのみ有効）

$$
C_{CO_2}^{*} = p^{CO_2} \cdot \left(
  \alpha_{ICE} \sum_{k \in \mathcal{K}^{ICE},\,j} f_k(j) \cdot y_j^k
  + \alpha_{grid} \sum_{t \in \mathcal{T}} g_t
\right)
$$

| 記号 | 意味 | 型 |
|------|------|----|
| $p^{CO_2}$ | CO₂ 価格（円/kg） | パラメータ（scenario.co2_price_per_kg） |
| $\alpha_{ICE}$ | 軽油の CO₂ 排出係数（kg/L、既定 2.64） | パラメータ |
| $\alpha_{grid}$ | 系統電力の CO₂ 排出係数（kg/kWh） | パラメータ（スロット別） |

**意味：** ICE 走行由来の CO₂ + 系統電力由来の CO₂ に価格を付ける。低炭素な充電時間帯・車種選択を誘導する。PV 自家消費分の CO₂ はゼロとする。

#### 電池劣化費（`degradation_weight > 0` のときのみ有効）

$$
C_{degr}^{*} = w^{degr} \cdot \sum_{k \in \mathcal{K}^{BEV},\,t \in \mathcal{T}} \frac{c_{k,t} \cdot \Delta t}{cap_k} \cdot \beta
$$

| 記号 | 意味 | 型 |
|------|------|----|
| $c_{k,t}$ | 車両 $k$ のスロット $t$ での充電電力（kW） | 連続（決定変数） |
| $\Delta t$ | スロット幅（h） | パラメータ |
| $cap_k$ | 車両 $k$ のバッテリー容量（kWh、既定 300） | パラメータ |
| $w^{degr}$ | 劣化費の重み（無次元） | パラメータ |
| $\beta$ | 1 サイクル当たり劣化コスト（円/cycle、現在 50.0 固定） | パラメータ |

**意味：** 充電量に比例するコストを加え、必要以上の充電（過充電）を抑制する。将来的には $\epsilon$-制約法または重み付き和で多目的化を検討。
