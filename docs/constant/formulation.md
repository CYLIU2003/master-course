...
> **⚠️ 注意（2026-03-18 追記）**
>
> **本文書は研究として最終的に目指す目標定式化を記述したものです。**
> 現時点（2026-03-18）の core 実装はその一部を先行実装しており、一部は未達成または近似です。
>
> - 実装済み範囲・部分対応・未対応の詳細は [`docs/constant/implementation_status.md`](implementation_status.md) を参照してください。
> - 現行実装の対応関係（制約 C1〜C21、目的関数 O1〜O4）は `README.md` の「10章」にも記載しています。
>
> **定式化（本文書） ≠ 実装済み範囲** であることに留意してください。
