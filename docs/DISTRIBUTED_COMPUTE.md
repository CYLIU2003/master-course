...
## 2026-09-24 更新: SSH中断時の安全な復旧と投入前Prepared照合（未配置）

計算中の一時的なSSH timeout／ネットワーク切断は、同じ要求・同じattempt IDで最大1回だけ再試行する。認証拒否・ホスト鍵不一致では再試行しない。再試行後も結果が不明なattemptは `LOST` としてPC枠とGurobi予約を保持し、同じattemptの開始記録・成果物を照合する。新しいIDで自動再投入しない。接続失敗したworkerは次のSSH probeが成功するまで新規割当から外し、再確認間隔は失敗回数に応じて最大120秒まで広げる。

batchで `prepared_input_id` を指定した場合、APIは完全なscenarioと現在のscopeを照合して同じ不変Preparedを読み込む。IDが古い、欠落、または内容検証に失敗した場合はjobを作らずHTTP 409で停止し、最適化要求の中で再Prepareしない。Prepare完了後に入力が変わったときは、変更後の入力で別のclean SHAを固定し、12週すべてを新たにPrepareしてから実行する。

現行固定版 `b9d0c60b8dc6f72faa0eaf72969ccd91fa0c3614` では、12 taskの初回投入はjob作成前に拒否された。最初のtaskのHTTP 500は、shallow scenarioの空の `timetable_rows` を日付付き時刻表検証へ渡したAPI経路の不具合だった。Preparedファイルは12/12存在し、Preparedの時刻表SHAはcontractと一致する。scenario hashは2〜12月の11件が現行値と一致し、1月だけPrepared `1bc2a3713c205963` に対して現行値 `7adccacdf7a2c128` となり不一致だった。古い1月Preparedを流用しない。修正は隔離worktreeにあり、現行controllerには未配置。実際の計算はまだ開始していない。

機器管理の各PCには「このPCに割り当てる計算」がある。`Gurobi計算`、`Gurobi不要のALNS`、`GurobiとALNS`、`診断のみ`から親機で選び、親機のSQLiteに再起動後も保持する。Gurobi能力を管理者設定で有効にしていないPCではGurobiを含む選択肢を選べない。自動割当・配布先指定の両方に適用し、配布先指定時の不一致は親ジョブ作成前に拒否する。計算画面の配布先一覧も担当外を選択不可と表示する。変更時点で実行中のジョブは続行し、配布先が固定された待機ジョブと矛盾する変更は拒否する。診断タスクはどの担当でも利用できる。Gurobi能力の登録、WLS資格情報の配置、有効なEnv/Model試験はこの画面の担当選択とは別の確認である。現行のGurobi対応PCの既定担当は「GurobiとALNS」、非対応PCは「Gurobi不要のALNS」。
...
計算中のSSH操作でも、タイムアウトや一時的な接続切断だけは同じ操作・同じ試行IDで1回再試行する。認証拒否・ホスト鍵不一致は再試行しない。再試行後も失敗した子機はその場で割当対象から外し、新しいSSH確認が成功するまで次のジョブを割り当てない。開始済みジョブは `LOST` とPC枠・Gurobi予約を保持して同じ試行を照合し、別IDでの自動再投入はしない。監視確認と計算要求が並行していても、失敗後に開始済みの古い確認が成功扱いで上書きすることはない。

ジョブ投入後のSSH切断で `LOST` になった試行は、同じIDで再実行しない。親機の照合は、子機の開始記録がない場合、同IDの遅延投入を防ぐ記録を子機上で先に原子的に作る。その確証を受けた試行だけ `BLOCKED`（子機で未開始）とし、子機枠・未使用Gurobi予約を解放して別IDでの再試行を可能にする。すでに開始記録がある試行は成果物を照合し、開始途中など不明な試行は `LOST` と予約を保持する。画面は未開始を明示し、成果物ZIPがない試行にはダウンロードを表示しない。Gurobiを持たない子機へのライセンス起動試験は投入時に拒否する。

22:20 JSTの更新: 固定版 `e301abbd` を18/18台へ照合配置し、親機の常駐版も切り替えた。役割はGurobi 5台・ALNS 13台、Gurobi共有枠は総数2・外部予約1、実行中ジョブ0件。`DESKTOP-5B6F6BP` への診断ジョブが完了し、回収ZIPのSHA-256が一致した。同機で未開始IDを封止し、遅延投入が拒否されることも実機確認した。[引継ぎと未完了事項](notes/CLUSTER_JOB_CONTROL_HANDOFF_20260923.md)。監査原本は `output/cluster-deployment/onboard-20260923/activation-worker-fence-e301abbd-audit.json`。
...
21:59 JSTの更新: 計算画面でも担当外を選択不可にした画面資材を配信した。配信JavaScriptと配置資材のSHA-256一致、画面テスト12件、本番ビルドを確認。バックエンド固定版は `91776624`、画面変更コミットは `b2c7853d`。常駐設定は `output/cluster-deployment/onboard-20260923/controller-worker-roles-ui-b2c7853d-settings.json`、稼働確認は同所の `activation-worker-roles-ui-b2c7853d-audit.json`。18台の担当設定と共有ライセンス枠は維持している。
...
渋24実便18件の現行batchは `output/cluster-deployment/onboard-20260923-add2/real-shibu24-18-0d056430-batch.json`。新シナリオ `993c2ec1-3333-442e-83ef-508df7aea667` の224便Prepared入力を固定版 `0d056430` で再照合し、Prepared ID・入力SHA-256が旧版の監査記録と一致することを確認したうえで `batch.py check` を通した。実便計算は未開始で、旧16台の結果は流用しない。投入時には各PCの空きRAM・CPU負荷・AC電源を再確認する。特に `LAPTOP-BOLC6VIT` はWindows Update中に空きRAMが1.64 GBまで低下したため、更新終了と2 GB以上の空きRAMを再確認するまで割当を待つ。旧版の検証記録は `output/cluster-deployment/onboard-20260923-add2/activation_audit.json`。
