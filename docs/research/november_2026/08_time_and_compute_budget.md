...
| RAIN 2×2 profiles | 4 | candidate/rollingを含む | 11,910 s |
| PV LOW/MEDIUM/HIGH（承認時のみ） | 3 | candidate/rollingを含む | 5,445 s |
| 合計 | 10 | artifactで実数保存 | 19,155 s = 5.32 h |

RAINの上限は `Stage1 + candidate_limit * Stage2 cap + 24 * rolling 30 s + 300 s orchestration reserve` でprofile JSONへ事前固定した。実際の共有budgetや早期終了があっても、上限を後から増やさない。PVはBASE相当として見積もる。
