...
新計算 `27253fa8` は18:57 JSTに起動し、入力候補作成直後の
`BUILDING_SOURCE_CANDIDATE` を旧observerが停止状態と誤判定した。
計算は継続しており、監視は開始前の検査で停止しただけ。計算は再起動しない。
制御側observerにこの正常初期状態を追加し、完了週が0であることを必須にした。
固定計算ソースは変更せず、初回binding前のhelper hashだけを再登録する。
