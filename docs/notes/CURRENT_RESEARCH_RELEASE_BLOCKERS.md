...
2026-09-23 翌朝SOC設定: 運用上限SOCを翌朝出庫前までに満たす方針と、最終夜間を計算に含める／含めない選択はシナリオへ保存できる。ただし翌日ダイヤ・PV・料金・充電・受電・費用の全区間が現行求解/検証に未接続のため、新設定での投入は `NEXT_MORNING_SOC_NOT_READY` で拒否する。旧設定の結果を新方針の証拠に読み替えない。設定と実行の差は [開発進捗](../../DEVELOPMENT_NOTES.md) を参照。
