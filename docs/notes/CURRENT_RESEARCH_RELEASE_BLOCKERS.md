...
2026-09-24のユーザー方針訂正: 厳密最適性証明と正式研究採用のBLOCKED判定は保存するが、
それだけを理由に週次の費用・季節比較を停止しない。固定7cb46894の12週を原本hashと
確定会計で再照合し、旧条件に限った `VERIFIED_CONDITIONAL_WEEKLY_EVALUATION` を別表示した。
物理・会計不成立を解除する変更ではない。新版の翌朝SOC条件、実受電設備の保証、
正式fleet承認、統合最適性を旧結果から認定しない。
[今回の再利用範囲と実行手順](WEEKLY_SEASONAL_EXECUTION_20260924.md)。

2026-09-23 翌朝SOC設定: 7日間の実便と検証済み8日目ダイヤ・PV、明示した料金反復規則、最終夜間を含むシナリオだけ、運用上限SOCを翌朝出庫前までに満たす診断を接続した。翌朝入力が欠ける設定、最終夜間を含めない設定は `NEXT_MORNING_SOC_NOT_READY` で引き続き拒否する。Stage 1/2・独立SOC検証・日別台帳の小規模回帰は通過したが、渋24の実規模12週の求解・rolling最終夜間実行・研究採用は未完了。旧設定の結果を新方針の証拠に読み替えない。[入力・比較条件](SHIBU24_MONTHLY_OVERNIGHT_20260923.md)。
...
連続週BESS引継ぎの実行経路は実装済み。2025-01-20/01-27の別診断を新規Prepareし、1週目の検証済み実行終端だけを2週目初期値へ渡す。2週の実際の通過、全車両状態の継続、実設備上限の確認はまだ未証明。既存月別12週の研究採用判定は変えない。
...
2026-09-21新方針: PV→バス優先・余剰PV→BESS・下限待機の補助制御を実装、関連222 tests通過。
Stage2/実行の小規模整合は確認したが、新方針の実規模・週間実績・最適性・独立承認は未確認。
Stage1連続recourseでは補助制御規則を緩和しており、Stage1/2の精度だけで統合総費用最適としない。
旧条件の出力を新方針へ転用しない。新5月設定は実行停止中。研究採用BLOCKED。
[適用範囲](BESS_AUXILIARY_POLICY_20260921.md)。

2026-09-21最新: 固定3dcdbffdのBESS20–80%対10–90%は両条件memory_limit、Stage1 gap3.545197%/3.559047%で1%未達。
修正後CO₂の新規実規模診断、4案の独立物理・BESS収支・会計、共通入力/hashの確認は完了。
予測総費用4,147,020.02/4,147,615.57円、206車両日。安い20–80%案を10–90%でも変更せず検証できたため、595.55円差を緩和の悪化効果とは扱わない。
週間統合最適性、修正後の週間実績・12週、入力仮定と10–90%実機適用範囲、独立研究レビューは未確認。
研究採用BLOCKED。新条件の全月展開は開始しない。[実測と判断](BESS_RANGE_SENSITIVITY_20260921.md)。以下は過去時点の記録。
...
## 2026-09-21 NoRel候補が営業日とDutyの検査不整合で停止

固定48ffb374は翌日路線変更をDuty全体の混在と判定しDAY_AHEAD_FAILED。保存候補の独立physical再読込みは違反0だが、修正後の新規求解は別途必要。Stage1差132.52円を採用済み削減額とせず、gap3.775968%・根LP未完了・最大24.097GB（soft18GB超過）を残す。日別検査と失敗理由を修正し、BESS追加予備・復元なしのまま新規5月診断へ進む。全12週・研究採用・独立承認は未完了。[根拠](ROUTE_BAND_SERVICE_DAY_FIX_20260921.md)。
...
ユーザー指定により、今後はBESSの設備20–80%範囲だけを守り、初期残量の追加予備と終端復元を外す。新設定と関連136 testsを確認したが、実際の週間費用・全12週・最適性は未評価。初期在庫の取り崩しを費用改善と混同せず、初期/最終kWhを併記する。実行中00aed18eは旧条件の表現比較として保持。研究採用BLOCKED。[方針](BESS_OPERATING_RANGE_POLICY_20260921.md)。

## 2026-09-21 入力経路は復旧、配車の最適化品質は未達

固定4f979982は両条件の物理・Stage2 gap0.368%を通過したが、配車の初期解改善0円・認証gap6.91%。dualは根LP未完了で600秒、NoRelは19.882GBで停止。旧12週の費用から改善を主張しない。充電可能時間の等価な端点表現を追加し、別固定版でdense表現と同条件比較する。構造削減だけを費用改善や最適性と扱わず、独立研究承認・実績費用・全12週は未完了。[根拠](STAGE1_CHARGE_WINDOW_SUPPORT_20260921.md)。
...
## 2026-09-21 計画整合性は通過、配車の精度は未達

固定3662b81aの5月前日計画は物理検証とDA/配車緩和のBESS保護672区間を通過。充電gap0.368%は目標1%以内。配車は初期解改善0円・gap47.87%で未達のため、十分に最適化した結果として研究に使わない。新しい完全網の日別経路被覆下界と根探索2条件を別固定版で診断する。実績168時間・全12週・独立研究承認は未完了。[根拠](STAGE1_ROOT_SEARCH_DIAGNOSIS_20260921.md)。

## 2026-09-21 計画と実行の整合修正後の判定

新ポリシーでBESS保護をDA・Stage1連続緩和・rollingの全発行ブロックへ揃えた。人工例で旧不具合を再現し、修正後の解消を確認した。旧68f2f4e5の費用を修正後の証拠として流用しない。実際の月別費用の改善、新条件の全12週、独立研究レビューは未完了。研究採用BLOCKED。[修正と検証範囲](OPTIMIZATION_CONSISTENCY_FIX_20260921.md)。
...
季節差の解釈には、今回の距離ベース需要が固定電費・燃費と倍率1.0で、気温に応じた空調負荷の月別変化を入力していない点を追加明記した。PV履歴・予測と配車・充電の対応の比較として扱い、冷暖房等を含む季節的需要変化の評価には拡張しない。計算設計は変更していない。
...
集計入口のP2テスト不足には合成資料10件を追加し、既存11件と合わせて21件通過。原本hash・SHA・時間数・区間数・物理・台帳・電力収支・campaign完了の検査を通過しない資料を最終集計へ入れないことを確認した。これはreport処理の検証であり、全12週の求解・独立監査や研究採用条件の代替ではない。

後続の本文生成では、費用差の符号付き内訳、指標ごとの順位、季節内の購入量範囲、BESS在庫利用を明示する。関連23件と仮データでの図の表示を検証したが、実結果の全12週レポート・図の確認は未完了。研究採用判定は変わらない。

## 2026-09-14 月別12週：入力の公平性を検査、週間結果は未確定

選択週の[日射量の代表性](SHIBU21_23_MONTHLY_IRRADIANCE_CONTEXT_20260914.md)も確認した。3月は月全体の日平均より35.2%少なく、10月は20.2%多い。曜日構成を揃える契約は満たすが、選択週を月平均・年平均の運用費用へそのまま外挿する根拠にはならない。選択週を結果に合わせて変更せず、条件差として報告する。

途中更新: 1〜3月は各168時間・672 slot、物理違反0件、会計・日別台帳・source hash照合を通過。4月はhour 023のStage 2がinfeasibleを返し、`STOPPED_AFTER_FAILED_CASE` で停止した。**3/12週の完走、4月は23/168時間、5〜12月は未実行**。原因の特定と対処が追加の実行継続条件となる。失敗週の費用を0やday-ahead費用で補わない。[確定した月の結果](SHIBU21_23_MONTHLY_RESULTS_20260914.md)。以下の「未完了」は12週全体の条件を指す。

祝日なし・月内完結の月曜〜日曜を各月から選び、全12週のmaterializationで平日5日・土曜1日・日曜1日、同一原本・1,704便・営業便距離・2024年予測モデルを確認した。旧秋の文化の日を含む週は11月10日開始へ変更した。新しいclean凍結版 `4c5c5d86` から逐次実行を開始した。完全Prepare・day-ahead・各168時間rolling・物理・会計・独立照合は未完了で、計算完了はまだ主張しない。[12週選択と検査契約](SHIBU21_23_MONTHLY_FAIR_WEEKS_20260914.md)。

曜日構成を揃えても割当や予測誤差等の差は残り、各月1週から季節一般・年平均・因果効果を主張しない。研究状態はDIAGNOSTIC / NOT USED FOR RESEARCH CONCLUSIONS、研究採用BLOCKED。以下の旧4週の完走証拠・gap・fleet来歴・資料の制限を引き継ぐ。
...
[四季の記述的比較](SHIBU21_23_SEASONAL_INTERPRETATION_20260912.md)を追加した。PV利用・抑制、購入電力・ピーク、使用車両日数の費用差を区別し、秋の総費用の低さをPV効果と解釈しない。営業便距離での正規化も因果交絡の解消ではない。BESSの初期在庫減少、有料契約超過、設備費計上0、各季節1週という条件を明示したが、下記の研究採用条件は未解消。
...
凍結 `a812aeb2` の冬・春は各168時間と独立物理検証を通過したが、春の未設定契約超過単価が0円として集計され、15,522.689810527645円の過少計上を検出した。旧春の会計成立を撤回し、夏のPrepare中に停止した。Builder・native solver・Evaluatorの未指定単価を500 JPY/kWhへ統一し、確定会計に超過量・料金の独立照合を追加する。保持計画の診断再集計は物理量不変・日別差0円で一致した。

修正後の回帰・独立レビューと新clean SHAによる全四季の新規Prepare・168時間・最終物理会計を確認する。以下の旧会計通過記録は当時の検査結果であり、今回の料金不一致に対する検証証拠ではない。最適性・fleet来歴・評価日と時刻表の年代・PV推定・資料証拠の既存制限も残り、研究リリースBLOCKEDを継続する。[原因・数式・検証](CONTRACT_OVERAGE_PRICE_DEFAULT_20260912.md)。

## 2026-09-12 三季節完走、秋159時間後の契約超過検証不一致

修正後の全体回帰2,228 passed / 既存資料2 failed、関連41件通過、独立コードレビュー残P0/P1は0件。保持秋状態から159〜167時の9回は通過した。新しいclean SHAでの全四季Prepare・168時間・最終物理会計は未完了。

凍結 `43fe848f` では冬・春・夏の全168時間、独立物理、最終会計が成立し、独立監査のP0/P1は0件。秋は159時間通過後、許可された有料契約超過を共通検証が拒否し停止した。秋の週間費用・最終会計は未成立。明示soft policyと超過量の計上整合を検証する修正後、全四季を新しいclean SHAで再実行する必要がある。旧三季節を新SHAへ流用しない。最適性ギャップ、正式fleet来歴、2026時刻表と2025評価日、2024年だけの予測学習、既存PowerPoint証拠2件の限界は残り、研究リリースはBLOCKED。[原因・変更・検証](CONTRACT_OVERAGE_VALIDATION_20260912.md)。
...
## 2026-09-11 全接続表現の実装完了、許可済み3路線の新実行待ち

全候補を維持した日跨ぎ接続の共有ノード表現を実装し、BEV/ICEを含む小規模native4ケースで元表現との費用一致と物理検証を確認した。4路線実入力の候補271,864,980本は除外0本で接続変数15,503,580個となったが、週間求解は未完了である。ユーザーが許可した渋21/22/23へ変更し、60台・7日間・SOC/BESS・全候補を保つ別designでfresh Prepareから順次実行する。新SHAの週間実行・168時間rolling・最終会計はまだ未確認であり、研究リリースBLOCKEDを継続する。旧停止記録は旧SHAの証拠として保持する。[定式化と検証](EXACT_DEPOT_CONNECTION_FACTORS_20260911.md)。
...
## 2026-09-11 Prepare性能修正後の再凍結

地点照合の反復を計算単位のキャッシュで削減した。全候補・式・ルール値を維持し、同等性・全体回帰・Lunaの対象レビューを確認した。これは研究採用の通過ではない。`737e07be` の実行は別成果物として保持し、修正後のSHAの診断には完全Prepareから新しく得た結果だけを使う。既存PowerPoint証拠2件と正式研究採用の制限は継続する。
