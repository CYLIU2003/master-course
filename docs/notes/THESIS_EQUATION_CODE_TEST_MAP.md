...
| BEV trip energy and ICE trip fuel | `trip_energy_kwh`, `fuel_l_by_vehicle_type`, `trip_energy_proxy.py`; assignment-linked integrated expressions; independent `bev_trip_energy_sensitivity_scale` and `ice_trip_fuel_sensitivity_scale` | `tests/test_trip_energy_proxy_and_location_aliases.py`, `tests/test_problem_builder_timestep_and_pv_scaling.py`, matrix/Prepare/accounting suites | BEV distance/duration and ICE distance/peak-time proxy allocation plus independent factors are tested. The common factor is retained only for shared demand calibration. Empirical route/direction coefficients are not available, and the two one-factor 0.8--1.2 solve tranches remain unexecuted at current HEAD |
...
4. Execute the predeclared 15/30/60-minute, route-band, common-demand,
   independent BEV-energy, independent ICE-fuel, PV-scale, and CO2-cap
   matrices from a clean frozen commit using
