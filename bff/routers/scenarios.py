...
def normalize_depot_energy_asset_config(raw: Dict[str, Any], depot_id: str = "") -> Dict[str, Any]:
    ...
    row["bess_terminal_soc_target_ratio"] = (terminal_target / capacity) if capacity > 0.0 else 0.0
    ...
    row["bess_terminal_soc_deviation_penalty_yen_per_kwh"] = _coerce_non_negative_float(
        _first_present_from_mapping(
            row,
            "bess_terminal_soc_deviation_penalty_yen_per_kwh",
            "bessTerminalSocDeviationPenaltyYenPerKwh",
            "bess_terminal_soc_target_penalty_yen_per_kwh",
            "bessTerminalSocTargetPenaltyYenPerKwh",
        ),
        "bess_terminal_soc_deviation_penalty_yen_per_kwh",
    )
...
def _builder_defaults(
    ...
    return {
        ...
        "pvMarginalChargeCostYenPerKwh": overlay_cost.get(
    ...
    *,
    ...
    return {
        ...
        "simulationSettings": {
            ...
            "planningDays": int(builder_defaults.get("planningDays") or 1),
            "rollingLookaheadHours": builder_defaults.get('rollingLookaheadHours'),
            "bessBalancePeriod": builder_defaults.get('bessBalancePeriod') or 'daily',
            "pvInformationMode": builder_defaults.get('pvInformationMode') or 'historical_perfect_information',
            "timeStepMin": int(builder_defaults.get("timeStepMin") or 30),
            "timestepMin": int(builder_defaults.get("timestepMin") or builder_defaults.get("timeStepMin") or 30),
...
def update_quick_setup(scenario_id: str, body: UpdateQuickSetupBody) -> Dict[str, Any]:
    ...
    try:
        ...
        if body.pvMarginalChargeCostYenPerKwh is not None:
            overlay_cost["pv_marginal_charge_cost_yen_per_kwh"] = max(
                float(body.pvMarginalChargeCostYenPerKwh),
                0.0,
            )
        if body.pvCurtailPenaltyYenPerKwh is not None:
