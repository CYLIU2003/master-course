...
def test_rebuild_updates_excel_and_records_the_updated_artifact(tmp_path) -> None:
    run_dir = tmp_path / "updated-excel"
    shutil.copytree(source_run_dir("run_20260606_1559"), run_dir)
    ...
    assert "results.xlsx" in result.updated_files
    log = json.loads(
        (run_dir / "rebuild_reporting_log.json").read_text(encoding="utf-8")
    )
    assert "results.xlsx" in log["updated_files"]
