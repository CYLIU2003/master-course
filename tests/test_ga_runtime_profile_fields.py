...
def _tiny_problem():
    ...
    return ProblemBuilder().build_from_dispatch(
        context,
        scenario_id="ga_runtime_profile",
...
def test_ga_optimizer_exports_runtime_profile_fields(monkeypatch) -> None:
    monkeypatch.setattr(ga_engine, "partial_milp_repair", lambda problem, plan, config=None: plan)

    result = GAOptimizer().solve(
        _tiny_problem(),
        OptimizationConfig(
    ...
    assert "Prototype genetic search" in metadata["comparison_note"]
