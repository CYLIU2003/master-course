...
from catalog_update_app import DEFAULT_OPERATOR


class CatalogDependencyError(RuntimeError):
    """The optional data preparation distribution is not available."""


def _catalog_builder_module(name: str) -> Any:
    """Load the separated ETL implementation only when a command needs it."""
    library = _REPO_ROOT / "data-prep" / "lib"
    if library.is_dir() and str(library) not in sys.path:
        sys.path.insert(0, str(library))
    try:
        return importlib.import_module(f"catalog_builder.{name}")
    except ModuleNotFoundError as exc:
        raise CatalogDependencyError(
            f"Catalog ingestion requires the separate data-prep/lib/catalog_builder "
            f"distribution (missing module: {exc.name}). "
            "This core checkout does not include the full ingestion pipeline."
        ) from exc
...
def build_odpt_url(
    ...
    return _catalog_builder_module("odpt_fetch").build_odpt_url(
        resource_name, consumer_key, operator_id, extra_params=extra_params
    )


def _chunk_query_params(resource_name: str, snapshot_dir: Path) -> List[Dict[str, str]]:
    return _catalog_builder_module("odpt_fetch")._chunk_query_params(resource_name, snapshot_dir)


def normalize_odpt_snapshot(raw_dir: Path, out_dir: Path) -> Dict[str, Any]:
    return _catalog_builder_module("odpt_normalize").normalize_odpt_snapshot(raw_dir, out_dir)
...
async def run_fetch_odpt(args: argparse.Namespace) -> int:
    _catalog_builder_module("odpt_fetch")
    if args.build_bundle:
        _catalog_builder_module("odpt_normalize")
...
def run_sync_gtfs(args: argparse.Namespace) -> int:
    from catalog_update_app import (
        _get_or_load_bundle, _parse_resources, _require_sync_dependencies,
        _resolve_scenario_id, _sync_bundle_to_scenario,
    )

    _require_sync_dependencies()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    except CatalogDependencyError as exc:
