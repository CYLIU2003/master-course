...
TOLERANCE = 1.0e-6
NATIVE_COUNT = 169
DECLARED_DESIGN_PATH: Path | None = None
...
def verify_solver_controls(config: dict, design: dict, declared: dict) -> dict:
    """Compare saved controls to the frozen source design, not old release constants."""
    mapping = {"phase": "phase", "time_limit_sec": "day_ahead_wall_time_limit_sec",
               "stage1_time_limit_sec": "stage1_time_limit_sec",
               "stage2_time_limit_sec": "stage2_time_limit_sec", "random_seed": "seed",
               "gurobi_threads": "threads", "mip_gap": "mip_gap"}
    keys = set(mapping.values()) | {"rolling_hour_time_limit_sec", "successor_pruning",
    ...
    require(config.get("allow_postsolve_repair") is False and design["postsolve_repair"] is False
            and design["successor_pruning"] == 0, "Repair or pruning enabled")
    require(design["diagnostic_stop_after_day_ahead"] is False, "Monthly campaign stopped at day-ahead")
    return expected
...
def audit_week(week: str) -> dict[str, Any]:
    ...
    declared = read_json(DECLARED_DESIGN_PATH) if DECLARED_DESIGN_PATH else design
    expected_config = verify_solver_controls(config, design, declared)
...
def main() -> int:
    require(DECLARED_DESIGN_PATH is not None, "Use a version-bound deployment wrapper")
