...
ここには既存の実験CLIと根拠生成処理を置いています。
保存済みコマンド、Python import、凍結manifestから参照されるため、
この整理では本体のパスと内容を変えていません。以下は主要な入口の用途別案内です。
...
| カタログDB構築 | [build_tokyu_full_db.py](build_tokyu_full_db.py)、[build_tokyu_gtfs_db.py](build_tokyu_gtfs_db.py) |
